...
MARKET_STATE_SCHEMA = {
    ...
    'confidence': 'float64',
    # Sentiment integration (NS-USO canonical context)
    'sentiment_available': 'bool',
    'sentiment_status': 'string',
    'sentiment_summary_age_minutes': 'float64',
    'sentiment_market_age_minutes': 'float64',
    'sentiment_polarity': 'float64',
    'sentiment_uncertainty': 'float64',
    'sentiment_conviction': 'float64',
    'sentiment_conflict': 'float64',
    'sentiment_dominant_theme': 'string',
    'sentiment_bias': 'float64',
    'sentiment_event_shock': 'float64',
    'sentiment_alert_level': 'string',
    'sentiment_news_signal': 'float64',
    'sentiment_micro_shift': 'float64',
    'sentiment_change_velocity': 'float64',
    'sentiment_delta_polarity': 'float64',
    'sentiment_delta_uncertainty': 'float64',
    'sentiment_delta_conviction': 'float64',
    'sentiment_negative_company_count': 'float64',
    'sentiment_event_impact_count': 'float64',
    'sentiment_risk_multiplier': 'float64',
    'sentiment_exposure_multiplier': 'float64',
    'sentiment_regime_headwind': 'float64',
...
class MarketStateEngine:
    ...
    def integrate_sentiment_intelligence(self, base_market_state):
        """
        Integrate NS-USO sentiment as a bounded context layer.

        Sentiment can modulate risk/exposure/confidence, but cannot override
        the macro-market spine with unbounded behavior.
        """
        state = dict(base_market_state or {})
        now = datetime.now()
        sentiment = load_sentiment_context(now=now)

        summary_age = sentiment.get("summary_age_minutes")
        market_age = sentiment.get("market_age_minutes")
        polarity = float(sentiment.get("polarity", 0.0) or 0.0)
        uncertainty = float(sentiment.get("uncertainty", 0.0) or 0.0)
        conviction = float(sentiment.get("conviction", 1.0) or 1.0)
        conflict = float(sentiment.get("narrative_conflict", 0.0) or 0.0)
        bias = float(sentiment.get("sentiment_bias", polarity) or 0.0)
        event_shock = float(sentiment.get("event_shock_score", 0.0) or 0.0)
        news_signal = float(sentiment.get("news_signal_score", 0.0) or 0.0)
        micro_shift = float(sentiment.get("micro_shift_score", 0.0) or 0.0)
        change_velocity = float(sentiment.get("change_velocity", 0.0) or 0.0)
        ...
        if bool(sentiment.get("available", False)):
            ...
            if event_shock < 0.25 and polarity > 0.0 and uncertainty < 0.45 and conflict < 0.45:
                tailwind = min(0.08, 0.08 * polarity + 0.06 * max(0.0, 0.45 - uncertainty))
                risk_multiplier = min(1.08, risk_multiplier + tailwind)
        ...
        if alert_level in {"high", "critical"}:
            confidence_penalty = min(0.22, confidence_penalty + 0.03)
        adjusted_confidence = max(0.05, min(1.0, base_confidence - confidence_penalty))

        state["risk_on_probability"] = adjusted_risk_on
        state["risk_on"] = adjusted_risk_on
        state["allowed_exposure"] = adjusted_exposure
        ...
        state.update(
            {
                ...
                "sentiment_summary_age_minutes": float(summary_age) if isinstance(summary_age, (int, float)) else np.nan,
                "sentiment_market_age_minutes": float(market_age) if isinstance(market_age, (int, float)) else np.nan,
                "sentiment_polarity": float(polarity),
                "sentiment_uncertainty": float(uncertainty),
                "sentiment_conviction": float(conviction),
                "sentiment_conflict": float(conflict),
                "sentiment_dominant_theme": str(sentiment.get("dominant_theme", "neutral") or "neutral"),
                "sentiment_bias": float(bias),
                "sentiment_event_shock": float(max(0.0, event_shock)),
                "sentiment_alert_level": alert_level,
                "sentiment_news_signal": float(max(0.0, news_signal)),
                "sentiment_micro_shift": float(max(0.0, micro_shift)),
                "sentiment_change_velocity": float(max(0.0, change_velocity)),
                "sentiment_delta_polarity": float(delta_polarity),
                "sentiment_delta_uncertainty": float(delta_uncertainty),
                "sentiment_delta_conviction": float(delta_conviction),
                "sentiment_negative_company_count": float(max(0, negative_count)),
                "sentiment_event_impact_count": float(max(0, event_count)),
                "sentiment_risk_multiplier": float(risk_multiplier),
                "sentiment_exposure_multiplier": float(exposure_multiplier),
                "sentiment_regime_headwind": float(sentiment_headwind),
            }
        )
    ...
    def compute_market_state(self):
        ...
        market_state = {
            'date': datetime.now(),
            # Legacy compatibility aliases (consumed by canonical state readers).
            'regime': macro_state['macro_regime'],
            'risk_on': risk_on_prob,
            'stress_score': vol_state['stress_level'],
            'macro_score': macro_state['macro_score'],
        ...
        # Integrate canonical sentiment context before validation.
        market_state = self.integrate_sentiment_intelligence(market_state)
    ...
    def save_market_state(self, state):
        ...
        # Append to existing data or create new (canonical write)
        try:
            if os.path.exists(self.output_path):
                ...
                self.state_manager.write_market_state(combined_df)
            else:
                self.state_manager.write_market_state(df)
        except Exception as e:
            print(f"⚠️ Error writing market state via StateFileManager: {e}")
            # Fallback to direct write if state manager fails
...
class IntelligentMarketStateEngine(MarketStateEngine):
    ...
    def validate_intelligent_consistency(self, state):
        ...
        ai_status = str(state.get('intelligence_status', '')).strip().lower()
        ai_active = ai_status == 'active'
        ai_regime_valid = str(ai_regime).strip().lower() not in {'', 'none', 'nan'}

        if ai_active and ai_regime_valid and base_regime != ai_regime:
