...
class PortfolioRiskController:
    ...
    def evaluate_convexity_budget(
        ...
        *,
        ...
        """
        Convexity and tail-budget controls for AlphaOS risk authority.
        """
        pv = max(float(portfolio_value), 1e-8)
        convexity_component = abs(float(gamma_exposure)) * max(float(underlying_vol), 0.0)
        vega_component = abs(float(vega_exposure)) * max(float(vol_of_vol), 0.0)
        liquidity_adjusted_vega = vega_component * (1.0 + max(0.0, float(liquidity_spread_pct)) * 5.0)
        convexity_score = float((convexity_component + liquidity_adjusted_vega) / pv)
        tail_convexity_score = float(
            (0.65 * convexity_score)
            + 0.25 * abs(float(delta_exposure)) / pv
            + 0.10 * max(0.0, float(gap_risk_score))
        )
        hard_limit = float(self.risk_params.get('max_drawdown', 0.12))
        veto = bool(tail_convexity_score > hard_limit)
        reasons = []
        if veto:
            reasons.append(f"tail_convexity_score_exceeded:{tail_convexity_score:.4f}>{hard_limit:.4f}")

        return {
            "convexity_score": convexity_score,
            "tail_convexity_score": tail_convexity_score,
            "gap_risk_score": float(gap_risk_score),
            "liquidity_adjusted_vega": float(liquidity_adjusted_vega),
            "veto_active": veto,
