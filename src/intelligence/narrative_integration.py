...
class NarrativeIntegration:
        ...
        try:
            ...
            if os.path.exists(self.data_paths['portfolio_allocations']):
                ...
                if not allocations_df.empty:
                    strategy_col = 'strategy_name' if 'strategy_name' in allocations_df.columns else ('strategy' if 'strategy' in allocations_df.columns else None)
                    weight_col = (
                        'allocation_weight'
                        if 'allocation_weight' in allocations_df.columns
                        else ('allocation' if 'allocation' in allocations_df.columns else ('weight' if 'weight' in allocations_df.columns else None))
                    )
                    date_col = 'date' if 'date' in allocations_df.columns else ('Date' if 'Date' in allocations_df.columns else None)
                    if strategy_col and weight_col:
                        latest_slice = allocations_df.copy()
                        if date_col:
                            ...
                            if not latest_slice.empty:
                                latest_date = latest_slice[date_col].max()
                        ...
                        cash_row = latest_slice[latest_slice[strategy_col] == 'CASH']
                        if not cash_row.empty:
                            market_data['cash_allocation'] = float(pd.to_numeric(cash_row.iloc[0][weight_col], errors='coerce') or 0.0)
                        else:
                            invested = float(pd.to_numeric(latest_slice[weight_col], errors='coerce').fillna(0.0).sum())
                            market_data['cash_allocation'] = float(np.clip(1.0 - invested, 0.0, 1.0))

                        # Total strategies
                        strategy_count = len(latest_slice[latest_slice[strategy_col] != 'CASH'])
            ...
            if os.path.exists(self.data_paths['strategy_beliefs']):
                ...
                if not beliefs_df.empty:
                    confidence_col = 'confidence' if 'confidence' in beliefs_df.columns else None
                    market_data.update({
                        'avg_belief_strength': float(pd.to_numeric(beliefs_df.get('belief_strength', 0.6), errors='coerce').mean()),
                        'avg_confidence': float(pd.to_numeric(beliefs_df[confidence_col], errors='coerce').mean()) if confidence_col else 0.7
                    })
            ...
            if os.path.exists(self.data_paths['strategy_regret']):
                ...
                if not regret_df.empty:
                    if 'regret_score' in regret_df.columns:
                        market_data['avg_regret'] = float(pd.to_numeric(regret_df['regret_score'], errors='coerce').mean())
            ...
            if os.path.exists(self.data_paths['market_tensor']):
                ...
                tensor_raw = pd.read_parquet(self.data_paths['market_tensor'])
    ...
    def detect_portfolio_changes(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(self.data_paths['portfolio_allocations']):
                ...
                if not allocations_df.empty and strategy_col and weight_col:
                    latest_slice = allocations_df.copy()
                    if date_col:
                        latest_slice[date_col] = pd.to_datetime(latest_slice[date_col], errors='coerce')
                        latest_slice = latest_slice.dropna(subset=[date_col])
                    ...
                    cash_allocation = float(pd.to_numeric(cash_rows.iloc[0][weight_col], errors='coerce')) if not cash_rows.empty else 0.0
                    ...
                    strategy_allocs = latest_slice[latest_slice[strategy_col] != 'CASH']
                    if not strategy_allocs.empty:
                        top_strategy = strategy_allocs.sort_values(weight_col, ascending=False).iloc[0]
                        portfolio_changes.append({
                            'action': 'allocated',
                            'asset_class': f'{top_strategy[strategy_col]} strategy',
                            'amount': f'{float(pd.to_numeric(top_strategy[weight_col], errors="coerce") or 0.0):.1%}',
    ...
    def generate_daily_narrative_enhanced(self) -> Dict[str, Any]:
        ...
        write_json_with_archive(self.output_paths['daily_narrative'], daily_narrative)
    ...
    def generate_weekly_pulse_enhanced(self) -> Dict[str, Any]:
        ...
        write_json_with_archive(self.output_paths['weekly_pulse'], weekly_pulse)
    ...
    def generate_monthly_report_enhanced(self) -> Dict[str, Any]:
        ...
        write_json_with_archive(self.output_paths['monthly_report'], monthly_report)
