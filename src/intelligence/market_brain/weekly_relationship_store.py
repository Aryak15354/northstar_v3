...
class WeeklyRelationshipStore:
    ...
    def _canonical_name(value):
        """Normalize relationship endpoint names."""
        s = str(value or "")
        ...
        try:
            ...
            if not df.empty:
                if 'src_name' in df.columns:
                    df['src_name'] = df['src_name'].map(self._canonical_name)
                if 'dst_name' in df.columns:
                    df['dst_name'] = df['dst_name'].map(self._canonical_name)
                if {'src_name', 'dst_name'}.issubset(df.columns):
                    df = df[df['src_name'] != df['dst_name']]

                # Remove lag-induced duplicates before persistence.
                key_cols = [c for c in ['date', 'src_name', 'dst_name', 'horizon'] if c in df.columns]
                if len(key_cols) == 4:
                    if 'confidence' in df.columns:
                        df['confidence'] = pd.to_numeric(df['confidence'], errors='coerce').fillna(0.0)
                    if 'information_gain' in df.columns:
                        df['information_gain'] = pd.to_numeric(df['information_gain'], errors='coerce').fillna(0.0)
                    df['_dedupe_score'] = (
                        df.get('confidence', 0.0).astype(float) +
                        0.5 * df.get('information_gain', 0.0).astype(float)
                    )
                    df = (
                        df.sort_values('_dedupe_score', ascending=False)
    ...
    def update_relationship_index(self, new_relationships, year, week):
        ...
        try:
            ...
            if index_entries:
                ...
                # Global dedupe across updates.
                dedupe_cols = ['date', 'src_name', 'dst_name', 'horizon', 'year', 'week']
                if all(c in index_df.columns for c in dedupe_cols):
                    if 'confidence' in index_df.columns:
                        index_df['confidence'] = pd.to_numeric(index_df['confidence'], errors='coerce').fillna(0.0)
                    if 'novelty' in index_df.columns:
                        index_df['novelty'] = pd.to_numeric(index_df['novelty'], errors='coerce').fillna(0.0)
                    index_df['_dedupe_score'] = index_df['confidence'] + 0.25 * index_df['novelty']
                    index_df = (
                        index_df.sort_values('_dedupe_score', ascending=False)
    ...
    def query_relationships(self, 
        ...
        try:
            ...
            if relationships:
                ...
                # Canonicalize endpoints to collapse duplicate tensor aliases.
                if 'src_name' in result.columns:
                    result['src_name'] = result['src_name'].map(self._canonical_name)
                if 'dst_name' in result.columns:
                    result['dst_name'] = result['dst_name'].map(self._canonical_name)
                if {'src_name', 'dst_name'}.issubset(result.columns):
                    result = result[result['src_name'] != result['dst_name']]

                # De-duplicate repeated lag-derived relationships.
                dedupe_cols = [c for c in ['date', 'src_name', 'dst_name', 'horizon'] if c in result.columns]
                if len(dedupe_cols) == 4:
                    ...
                    if 'information_gain' in result.columns:
                        result['information_gain'] = pd.to_numeric(result['information_gain'], errors='coerce').fillna(0.0)
                    result['_dedupe_score'] = result.get('confidence', 0.0) + 0.5 * result.get('information_gain', 0.0)
                    result = (
                        result.sort_values('_dedupe_score', ascending=False)
                        .drop_duplicates(subset=dedupe_cols, keep='first')
                        .drop(columns=['_dedupe_score'])
                    )
