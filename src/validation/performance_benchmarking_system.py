...
class PerformanceBenchmarkingSystem:
    ...
    def decompose_returns(self, portfolio_returns: pd.Series,
        ...
        # Uses numpy least-squares to avoid heavy optional dependencies.
        try:
            y = pd.to_numeric(aligned_port, errors="coerce")
            X = aligned_factors.apply(pd.to_numeric, errors="coerce")
            ...
            coeff, *_ = np.linalg.lstsq(X_mat, y.values, rcond=None)

            # Contribution share by absolute beta magnitude
            beta = np.abs(coeff[1:])
            ...
            for i, col in enumerate(X.columns):
                out[str(col)] = float(beta[i] / beta_sum)

            explained = min(1.0, max(0.0, float(np.var(X_mat @ coeff) / max(np.var(y.values), 1e-12))))
            out["Idiosyncratic"] = float(max(0.0, 1.0 - explained))
            return out
        except Exception:
            return {"Idiosyncratic": 1.0}
