...
DEFAULT_IN_FILE = "data/macro/factors/macro_factors.parquet"
DEFAULT_OUT_FILE = "data/macro/factors/macro_score.parquet"
...
def decorrelate_weights(factors_df: pd.DataFrame, base_weights: dict, shrinkage: float = 0.15) -> dict:
    """
    Correlation-aware weight adjustment.
    Mitigates unstable amplification when factors are highly collinear.
    """
    cols = ['G', 'I', 'L', 'S']
    f = factors_df[cols].apply(pd.to_numeric, errors='coerce').replace([np.inf, -np.inf], np.nan).dropna()
    if f.empty or len(f) < 30:
        return dict(base_weights)

    corr = f.corr().fillna(0.0).values
    eye = np.eye(len(cols))
    corr_shrunk = (1.0 - shrinkage) * corr + shrinkage * eye
    try:
        inv_corr = np.linalg.pinv(corr_shrunk)
    except Exception:
        return dict(base_weights)

    w = np.array([float(base_weights[c]) for c in cols], dtype=float)
    w_adj = inv_corr @ w
    if not np.isfinite(w_adj).all():
        return dict(base_weights)

    # Preserve overall leverage of the original vector.
    base_l1 = float(np.sum(np.abs(w)))
    adj_l1 = float(np.sum(np.abs(w_adj)))
    if adj_l1 > 1e-12:
        w_adj = w_adj * (base_l1 / adj_l1)

    return {c: float(v) for c, v in zip(cols, w_adj)}
...
def validate_regimes(df):
    ...
    if regime_changes > 0:
        ...
        print("  Average regime duration: N/A (no changes)")
...
def run(in_file: str = DEFAULT_IN_FILE, out_file: str = DEFAULT_OUT_FILE):
    ...
    # Correlation-aware weights reduce instability under high collinearity.
    use_decorrelation = os.getenv("MACRO_DECORRELATE", "1") != "0"
    active_weights = dict(WEIGHTS)
    if use_decorrelation:
        active_weights = decorrelate_weights(
            pd.DataFrame({'G': g, 'I': i, 'L': l, 'S': df['TrueStress']}),
            WEIGHTS,
        )
        print(
            "   Decorrelated weights: "
            + ", ".join(f"{k}={active_weights[k]:+.3f}" for k in ['G', 'I', 'L', 'S'])
        )

    # Per-factor contributions (help detect sign/scale issues)
    df['Contrib_G'] = active_weights['G'] * g
    df['Contrib_I'] = active_weights['I'] * i
    df['Contrib_L'] = active_weights['L'] * l
    df['Contrib_S'] = active_weights['S'] * df['TrueStress']
    ...
    if df.empty:
        print("⚠️ No valid MacroScore rows after cleaning; writing empty macro_score artifact.")
        # Preserve required columns for downstream consumers.
        out_cols = ['MacroScore', 'Regime', 'Regime_Momentum']
        out = pd.DataFrame(columns=out_cols)
        out.to_parquet(out_file)
        print(f"✅ Saved empty macro score frame: {out_file}")
        return
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build macro regime score and labels from macro factors")
    parser.add_argument("--in-file", type=str, default=DEFAULT_IN_FILE)
    parser.add_argument("--out-file", type=str, default=DEFAULT_OUT_FILE)
