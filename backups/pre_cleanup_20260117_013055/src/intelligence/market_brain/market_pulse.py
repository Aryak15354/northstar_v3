...
class MarketPulseEngine:
    ...
    def load_market_brain_components(self):
        ...
        try:
            if os.path.exists(self.paths['market_tensor']):
                components['tensor'] = pd.read_parquet(self.paths['market_tensor'])
    ...
    def detect_dominant_forces(self, tensor):
        ...
        for column in tensor.columns:
            try:
                ...
                trend_strength = abs(np.corrcoef(range(len(recent_values)), recent_values)[0, 1])
                
                # Combine into force strength
                force_strength = (recent_change * 0.4 + volatility * 0.3 + trend_strength * 0.3)
    ...
    def match_current_regime(self, tensor, regime_fingerprints):
        ...
        try:
            ...
            from src.regime_memory import RegimeMemoryEngine
    ...
    def synthesize_market_pulse(self, dominant_forces, propagation_map, regime_info):
        ...
        total_force_strength = sum(f['strength'] for f in dominant_forces.values())
        pulse_intensity = min(total_force_strength / len(dominant_forces), 2.0) if dominant_forces else 0.0
