...
# Try to import TensorFlow for autoencoder, fallback to PCA if not available
TF_AVAILABLE = False
try:
    ...
    TF_AVAILABLE = True
    print("✅ TensorFlow available with Metal GPU acceleration for advanced regime compression")
...
class RegimeMemoryEngine:
    ...
    def build_autoencoder(self, input_dim):
        """Build temporal autoencoder for regime compression"""
        
        if not TF_AVAILABLE:
            print("   Using PCA instead of autoencoder")
            return None
        
        print("🧠 Building temporal autoencoder...")
        ...
        print(f"   🏗️ Autoencoder: {input_dim} → {embedding_dim} → {input_dim}")
