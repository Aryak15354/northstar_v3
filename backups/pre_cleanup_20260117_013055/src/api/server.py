...
async def get_timeseries(name: str, limit: int = 500) -> Dict[str, Any]:
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            mock_data = create_mock_timeseries(name, limit)
            return {
                "success": True,
                "data": mock_data,
                "count": len(mock_data),
...
async def get_strategies() -> Dict[str, Any]:
    ...
    try:
        ...
        if not strategies_data:
            strategies_data = create_mock_strategies()
...
async def get_portfolio_holdings(limit: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        if os.path.exists(weights_path):
            ...
            mock_holdings = create_mock_holdings(limit)
            return {
                "success": True,
                "data": mock_holdings,
                "count": len(mock_holdings),
                "timestamp": datetime.now().isoformat(),
                "source": "mock"
            }
    ...
    try:
        ...
        if os.path.exists(chart_data_path):
            ...
            if os.path.exists(pnl_path):
                ...
                mock_chart_data = create_mock_pnl_chart()
                return {
                    "success": True,
...
def create_mock_timeseries(name: str, limit: int):
    ...
    if name == 'pnl':
        ...
        return [
            ...
            for date, eq, ret in zip(dates, equity, returns)
        ]
        ...
        return [
            {
                'date': date.isoformat(),
                'macro_regime': np.random.choice(['Risk-On', 'Risk-Off', 'Neutral']),
                'risk_on_probability': np.random.uniform(20, 80),
                'allowed_exposure': np.random.uniform(60, 90)
            }
            for date in dates
        ]
    else:
...
def create_mock_strategies():
    ...
    strategies = ['momentum_6m', 'value_blend', 'quality_growth', 'low_vol', 
                 'mean_reversion', 'sector_rotation', 'earnings_momentum', 'technical_breakout']
    
    return {
        'beliefs': {f"{strategy}_skill": np.random.beta(2, 2) for strategy in strategies},
        'regret': {f"{strategy}_regret": np.random.exponential(0.1) for strategy in strategies},
        'capital': {
            'allocations': {strategy: np.random.uniform(0.05, 0.2) for strategy in strategies}
        }
    }

def create_mock_pnl_chart():
    """Create mock P&L chart data"""
    ...
    cumulative_return = (equity / equity[0] - 1) * 100
    peak = pd.Series(equity).expanding().max()
    drawdown = (pd.Series(equity) / peak - 1) * 100
    
    return {
        'dates': [date.strftime('%Y-%m-%d') for date in dates],
        'equity': [round(eq, 0) for eq in equity],
        'cumulative_return': [round(ret, 2) for ret in cumulative_return],
        'drawdown': [round(dd, 2) for dd in drawdown],
        'metadata': {
...
def create_mock_holdings(limit: int):
    """Create mock portfolio holdings"""
    tickers = ['RELIANCE', 'TCS', 'HDFCBANK', 'INFY', 'HINDUNILVR', 'ICICIBANK', 
    ...
    for i, ticker in enumerate(tickers[:limit]):
        holdings.append({
            'ticker': ticker,
            'weight': np.random.uniform(0.01, 0.08),
            'final_weight': np.random.uniform(0.01, 0.08),
            'sector': np.random.choice(['Technology', 'Finance', 'Consumer', 'Energy', 'Healthcare'])
        })
