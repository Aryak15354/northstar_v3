...
# Input files
ENGINE_WEIGHTS = "data/processed/scores.parquet"  # Using existing Northstar scores as proxy
...
def load_engine_weights():
    ...
    print("📊 Loading engine weights from Northstar scores...")

    if not os.path.exists(ENGINE_WEIGHTS):
        print(f"❌ Engine weights not found: {ENGINE_WEIGHTS}")
        print("   Run the complete pipeline first: python src/processing/pipeline.py")
        raise FileNotFoundError(f"Required file not found: {ENGINE_WEIGHTS}")

    # Load actual scores and convert to weights
    scores = pd.read_parquet(ENGINE_WEIGHTS)
    ...
    score_cols = [c for c in ['northstar_score', 'final_score', 'score', 'ns_score'] if c in scores.columns]

    if not ticker_col:
        raise ValueError("No ticker column found in scores data. Expected 'ticker' or 'Ticker'")
    
    if not score_cols:
        raise ValueError("No score column found in scores data. Expected 'northstar_score', 'final_score', 'score', or 'ns_score'")
...
def apply_macro_throttle(weights_df, risk_df):
    ...
    # Build sector map from scores file if available
    sector_map = {}
    try:
        scores = pd.read_parquet(ENGINE_WEIGHTS)
        tcol = 'ticker' if 'ticker' in scores.columns else ('Ticker' if 'Ticker' in scores.columns else None)
        icol = 'Industry' if 'Industry' in scores.columns else None
        if tcol and icol:
            tmp = scores[[tcol, icol]].dropna().drop_duplicates(subset=[tcol])
