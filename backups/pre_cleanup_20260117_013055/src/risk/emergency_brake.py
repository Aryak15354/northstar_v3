...
class EmergencyBrakeEngine:
    ...
    def load_portfolio_data(self):
        ...
        print("⚠️ Using synthetic portfolio data for emergency brake testing")
        dates = pd.date_range(start='2024-01-01', end='2024-12-31', freq='D')
        returns = np.random.normal(0.0005, 0.015, len(dates))  # 0.05% daily return, 1.5% vol
        
        # Add some stress periods
        stress_periods = [
            (100, 110),  # 10-day stress period
            (200, 205),  # 5-day stress period
        ]
        
        for start, end in stress_periods:
            if end < len(returns):
                returns[start:end] = np.random.normal(-0.02, 0.03, end-start)  # -2% daily, 3% vol
        
        equity = (1 + pd.Series(returns, index=dates)).cumprod()
