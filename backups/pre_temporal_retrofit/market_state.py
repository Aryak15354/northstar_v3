...
class MarketStateEngine:
    ...
    def integrate_market_brain_intelligence(self, base_market_state):
        ...
        try:
            ...
            if brain_intelligence['available']:
                ...
                if pulse_state:
                    enhanced_state_manager.update_state("component", {'pulse_intensity': pulse_state.get('pulse_intensity', 0.5)}, AuthorityLevel.SYSTEM, "State field update")
                    enhanced_state_manager.update_state("component", {'market_phase': pulse_state.get('market_phase', 'neutral')}, AuthorityLevel.SYSTEM, "State field update")
                    enhanced_state_manager.update_state("component", {'pulse_risk_level': pulse_state.get('risk_level', 'medium')}, AuthorityLevel.SYSTEM, "State field update")
                    
                    # Regime intelligence
                    regime_info = pulse_state.get('regime_info', {})
                    current_regime = regime_info.get('current_regime', {})
                    if current_regime:
                        enhanced_state_manager.update_state("component", {'regime_similarity': current_regime.get('similarity', 0.5)}, AuthorityLevel.SYSTEM, "State field update")
                        enhanced_state_manager.update_state("component", {'regime_name': current_regime.get('regime_name', 'Unknown')}, AuthorityLevel.SYSTEM, "State field update")
                
                # Survival intelligence
                survival_state = brain_intelligence.get('survival_state', {})
                if survival_state:
                    enhanced_state_manager.update_state("component", {'survival_mode': survival_state.get('survival_mode', 'normal')}, AuthorityLevel.SYSTEM, "State field update")
                    action_params = survival_state.get('action_parameters', {})
                    enhanced_state_manager.update_state("component", {'exposure_multiplier': action_params.get('exposure_multiplier', 1.0)}, AuthorityLevel.SYSTEM, "State field update")
                
                # Causal stability
                causal_stability = brain_intelligence.get('causal_stability', 0.7)
                enhanced_state_manager.update_state("component", {'causal_stability': causal_stability}, AuthorityLevel.SYSTEM, "State field update")
                
                enhanced_state_manager.update_state("component", {'brain_active': True}, AuthorityLevel.SYSTEM, "State field update")
    ...
    def apply_brain_adjustments(self, state):
        ...
        state_manager.update_state("component", {'allowed_exposure': max(5, min(original_exposure, brain_adjusted_exposure))}, AuthorityLevel.SYSTEM, "State field update")
        ...
        if survival_mode in ['emergency', 'shutdown']:
            state_manager.update_state("component", {'stress_level': max(state['stress_level'], 0.8)}, AuthorityLevel.SYSTEM, "State field update")
        elif survival_mode == 'stress':
            state_manager.update_state("component", {'stress_level': max(state['stress_level'], 0.6)}, AuthorityLevel.SYSTEM, "State field update")
    ...
    def validate_consistency(self, state):
        ...
        try:
            ...
            state_manager.update_state("component", {'coherence_score': coherence_score}, AuthorityLevel.SYSTEM, "State field update")
            state_manager.update_state("component", {'coherence_warnings': warnings}, AuthorityLevel.SYSTEM, "State field update")
        except Exception as e:
...
# print(f"⚠️ Intelligence stack not available: {e}")
    ...
    def compute_intelligent_market_state(self):
        ...
        if self.intelligence_available:
            try:
                ...
                intelligent_state_manager.update_state("component", {'intelligence_status': 'error'}, AuthorityLevel.SYSTEM, "State field update")
                intelligent_state_manager.update_state("component", {'intelligence_error': str(e)}, AuthorityLevel.SYSTEM, "State field update")
        else:
            # No intelligence available
            intelligent_state = base_state
            intelligent_state_manager.update_state("component", {'intelligence_status': 'unavailable'}, AuthorityLevel.SYSTEM, "State field update")
    ...
    def validate_intelligent_consistency(self, state):
        ...
        state_manager.update_state("component", {'consistency_warnings': warnings}, AuthorityLevel.SYSTEM, "State field update")
    ...
    def save_intelligent_state(self, state):
        ...
        if 'date' in parquet_state and isinstance(parquet_state['date'], datetime):
            parquet_state_manager.update_state("component", {'date': parquet_state['date']}, AuthorityLevel.SYSTEM, "State field update")
        elif 'date' not in parquet_state:
            parquet_state_manager.update_state("component", {'date': datetime.now()}, AuthorityLevel.SYSTEM, "State field update")
        
        if 'intelligence_timestamp' in parquet_state and isinstance(parquet_state['intelligence_timestamp'], datetime):
            parquet_state_manager.update_state("component", {'intelligence_timestamp_str': parquet_state['intelligence_timestamp'].isoformat()}, AuthorityLevel.SYSTEM, "State field update")
