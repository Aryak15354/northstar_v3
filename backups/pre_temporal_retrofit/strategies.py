...
def _load_scores() -> pd.DataFrame:
    ...
    # Industry enrichment from universe
    try:
        if (df['Industry'] == 'Unknown').mean() > 0.5 and os.path.exists(UNIVERSE_FILE):
            ...
            icol = 'Industry' if 'Industry' in uni.columns else ('Industry Name' if 'Industry Name' in uni.columns else None)
            if icol:
                u = uni[['ticker', icol]].dropna().drop_duplicates('ticker')
                df = df.merge(u, on='ticker', how='left', suffixes=('', '_u'))
                df['Industry'] = df['Industry'].where(df['Industry'] != 'Unknown', df[f'{icol}'].fillna('Unknown'))
                drop_cols = [c for c in df.columns if c.endswith('_u') or c == icol]
