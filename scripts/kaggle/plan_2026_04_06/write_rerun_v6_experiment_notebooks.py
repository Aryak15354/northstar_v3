#!/usr/bin/env python3
"""Write brand-new rerun-v6 Kaggle notebooks for EXP-09 through EXP-18."""
    ...
    return dedent(
        ...
        def add_cross_sectional_interactions(frame):
            ...
            specs = [
                ...
                ("vix_x_beta", ["india_vix_norm", "india_vix", "india_vix_z"], "bab_signal"),
            ]
        ...
        def run_preflight_checks(bundle, feature_cols):
            ...
            assert not forbidden, f"Precomputed CS transforms leaked into v6 features: {forbidden[:20]}"
...
RATIO_V2_NOTEBOOK = [
    _md(
        ...
        This notebook is the rerun-v6 campaign for the ratio experiments.
        ...
        - `min_data_in_leaf=1` and `l2_leaf_reg=3` are restored for EXP-09 v6
    ...
    _code(
        ...
        SANITY_PROBES = [
            {"label": "sanity_depth6_no_es", "params": {"depth": 6, "od_wait": 30, "l2_leaf_reg": 3, "min_data_in_leaf": 1, "learning_rate": 0.05, "iterations": 400, "boosting_type": "Ordered", "loss_function": "RMSE", "eval_metric": "RMSE", "random_seed": 42}, "use_early_stopping": False},
            {"label": "sanity_depth5_es", "params": {"depth": 5, "od_wait": 20, "l2_leaf_reg": 3, "min_data_in_leaf": 1, "learning_rate": 0.05, "iterations": 400, "boosting_type": "Ordered", "loss_function": "RMSE", "eval_metric": "RMSE", "random_seed": 42}, "use_early_stopping": True},
            {"label": "sanity_depth4_es", "params": {"depth": 4, "od_wait": 30, "l2_leaf_reg": 3, "min_data_in_leaf": 1, "learning_rate": 0.05, "iterations": 400, "boosting_type": "Ordered", "loss_function": "RMSE", "eval_metric": "RMSE", "random_seed": 42}, "use_early_stopping": True},
        ]
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v6]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_ratio_rerun_v6_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_ratio_rerun_v6_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v6", "exp10_v6", "exp09_v6"]:
        ...
        def run_exp09_v6():
            ...
            title = "EXP-09 v6: CatBoost Depth x Early Stopping"
            sanity_rows = []
            sanity_payloads = []
            ...
            if FORCE_STOP_ON_SANITY_FAIL and (not best_sanity or best_sanity["mean_test_ic"] < 0.020):
                write_text(
                    exp_root / f"{exp_slug}_verdict.txt",
                    "Sanity check suite failed: best sanity probe mean_test_ic below 0.020. Stopping EXP-09 before the grid.",
                )
                raise RuntimeError("Best sanity probe IC < 0.020. Dataset or feature pipeline issue must be resolved before EXP-09.")
            ...
            for depth, od_wait in EXP09_V2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v6 grid early.")
        ...
        def run_exp10_v6():
            exp_slug = "exp10_v6"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v6: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v6")
            if not best_exp09 or not best_exp09.get("gate_ic"):
                raise RuntimeError("EXP-10 v6 requires a qualified EXP-09 v6 result first.")
            rows = []
            for l2_value in EXP10_L2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v6 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
                        run_seed_validation(FRAME, FEATURE_COLS, bundle["splits"], params, exp_root, label, max_splits=MAX_SPLITS)
                    break
            qualified = [row for row in rows if row["gate_ic"]]
        ...
        def run_exp11_v6():
            exp_slug = "exp11_v6"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v6: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v6")
            if not best_exp10:
                raise RuntimeError("EXP-11 v6 requires EXP-10 v6 results.")
            rows = []
        ...
        def run_exp12_v6():
            exp_slug = "exp12_v6"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v6: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v6")
            if not best_exp11:
                raise RuntimeError("EXP-12 v6 requires EXP-11 v6 results.")
            rows = []
            regime_breakdowns = {}
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v6_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        ...
        This notebook is the rerun-v6 campaign for the sector, event, and collapse experiments.
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v6]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_sector_rerun_v6_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_sector_rerun_v6_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def run_exp13_v6():
            ...
            if "rbi_rate_chg" not in FULL_FRAME.columns:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v6.")
            ...
            write_json(exp_root / "exp13_v6_summary.json", payload)
            write_table(exp_root / "exp13_v6_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v6():
            ...
            if anchor_feature is None:
                raise RuntimeError("An FX anchor feature must be present for EXP-14 v6.")
            ...
            write_json(exp_root / "exp14_v6_summary.json", payload)
            write_table(exp_root / "exp14_v6_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v6():
            ...
            write_json(exp_root / "exp15_v6_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v6():
            ...
            write_json(exp_root / "exp16_v6_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v6():
            ...
            write_json(exp_root / "exp17_v6_summary.json", payload)
        ...
        def run_exp18_v6():
            exp_slug = "exp18_v6"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v6")
            if exp17_state is None:
                exp17_state = run_exp17_v6()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v6_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v6_20260406.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v6_20260406.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v6_20260406.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v6_20260406.ipynb"
