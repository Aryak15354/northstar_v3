#!/usr/bin/env python3
"""Write brand-new rerun-v8 Kaggle notebooks for EXP-09 through EXP-18."""
...
def _common_runtime(log_prefix: str) -> str:
    return dedent(
        ...
        def run_preflight_checks(bundle, feature_cols):
            ...
            assert not forbidden, f"Precomputed CS transforms leaked into v8 features: {forbidden[:20]}"
...
RATIO_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V6: EXP-09 through EXP-12

        This notebook is the rerun-v8 campaign for the ratio experiments.
    ...
    _code(
        ...
        EXP09_POLICY_GRID = [
            ...
            {"label": "depth_5_roll2_no_es", "depth": 5, "od_wait": 20, "override_window_years": 2},
        ]
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v8]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_ratio_rerun_v8_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_ratio_rerun_v8_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v8", "exp10_v8", "exp09_v8"]:
        ...
        def run_exp09_v8():
            ...
            title = "EXP-09 v8: Raw-Feature Depth x Lookback Policy"
            sanity_rows = []
            sanity_payloads = []
            regime_breakdowns = {}
            ...
            for spec in EXP09_POLICY_GRID:
                ...
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v8 grid early.")
        ...
        def run_exp10_v8():
            exp_slug = "exp10_v8"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v8: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v8")
            if not best_exp09:
                raise RuntimeError("EXP-10 v8 requires EXP-09 v8 results first.")
            if not best_exp09.get("gate_ic") and not ALLOW_BEST_AVAILABLE_CHAIN:
                raise RuntimeError("EXP-10 v8 requires a qualified EXP-09 v8 result when ALLOW_BEST_AVAILABLE_CHAIN is disabled.")
            ...
            for l2_value in EXP10_L2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v8 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
        ...
        def run_exp11_v8():
            exp_slug = "exp11_v8"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v8: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v8")
            if not best_exp10:
                raise RuntimeError("EXP-11 v8 requires EXP-10 v8 results.")
        ...
        def run_exp12_v8():
            exp_slug = "exp12_v8"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v8: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v8")
            if not best_exp11:
                raise RuntimeError("EXP-12 v8 requires EXP-11 v8 results.")
            rows = []
            regime_breakdowns = {}
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v8_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V6: EXP-13 through EXP-18

        This notebook is the rerun-v8 campaign for the sector, event, and collapse experiments.
    ...
    _code(
        ...
        TREE_PARAM_OVERRIDES = {}
        EXP13_FS_CONFIGS = [
            {"label": "fs_depth3_no_es", "depth": 3, "l2_leaf_reg": 4},
            {"label": "fs_depth4_no_es", "depth": 4, "l2_leaf_reg": 4},
        ]
        EXP13_LENDERS_CONFIGS = [
            {"label": "lenders_depth2_no_es", "depth": 2, "l2_leaf_reg": 8, "iterations": 250},
            {"label": "lenders_depth3_no_es", "depth": 3, "l2_leaf_reg": 8, "iterations": 300},
        ]
        EXP13_OTHER_FIN_CONFIGS = [
            {"label": "other_fin_depth2_no_es", "depth": 2, "l2_leaf_reg": 6, "iterations": 250},
            {"label": "other_fin_depth3_no_es", "depth": 3, "l2_leaf_reg": 6, "iterations": 300},
        ]
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v8]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_sector_rerun_v8_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_sector_rerun_v8_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def tuned_params_for_universe(base_params, ticker_count):
        ...
        def run_config_sweep(frame, feature_cols, params_list, label_prefix, exp_root, max_splits=None):
            ...
            for spec in params_list:
                params = tuned_params_for_universe({**BASE_PARAMS, **spec}, int(frame["ticker"].nunique()))
                label = str(spec["label"])
                state = run_single_config(
                    frame,
                    feature_cols,
        ...
        def run_exp13_v8():
            ...
            if "rbi_rate_chg" not in FULL_FRAME.columns:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v8.")
            ...
            write_json(exp_root / "exp13_v8_summary.json", payload)
            write_table(exp_root / "exp13_v8_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v8():
            ...
            if anchor_feature is None:
                raise RuntimeError("An FX anchor feature must be present for EXP-14 v8.")
            ...
            write_json(exp_root / "exp14_v8_summary.json", payload)
            write_table(exp_root / "exp14_v8_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v8():
            ...
            write_json(exp_root / "exp15_v8_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v8():
            ...
            write_json(exp_root / "exp16_v8_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v8():
            ...
            write_json(exp_root / "exp17_v8_summary.json", payload)
        ...
        def run_exp18_v8():
            exp_slug = "exp18_v8"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v8")
            if exp17_state is None:
                exp17_state = run_exp17_v8()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v8_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v8_20260407.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v8_20260407.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v8_20260407.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v8_20260407.ipynb"
