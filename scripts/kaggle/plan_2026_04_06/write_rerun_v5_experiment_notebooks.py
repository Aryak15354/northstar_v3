#!/usr/bin/env python3
"""Write brand-new rerun-v5 Kaggle notebooks for EXP-09 through EXP-18."""
...
def _common_runtime(log_prefix: str) -> str:
    return dedent(
        ...
        def run_preflight_checks(bundle, feature_cols):
            ...
            assert not forbidden, f"Precomputed CS transforms leaked into v5 features: {forbidden[:20]}"
...
RATIO_V2_NOTEBOOK = [
    _md(
        ...
        This notebook is the rerun-v5 campaign for the ratio experiments.
        ...
        - `min_data_in_leaf=1` and `l2_leaf_reg=3` are restored for EXP-09 v5
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v5]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_ratio_rerun_v5_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_ratio_rerun_v5_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v5", "exp10_v5", "exp09_v5"]:
        ...
        def run_exp09_v5():
            ...
            title = "EXP-09 v5: CatBoost Depth x Early Stopping"
            sanity_params = {
                "depth": 6,
                "od_wait": 30,
                "l2_leaf_reg": 3,
            ...
            if FORCE_STOP_ON_SANITY_FAIL and sanity_state["aggregate"]["mean_test_ic"] < 0.020:
                write_text(
                    ...
                    "Sanity check failed: mean_test_ic below 0.020. Stopping EXP-09 v5 before the grid.",
                )
                raise RuntimeError("Sanity IC < 0.020. Dataset or feature pipeline issue must be resolved before EXP-09 v5.")
            ...
            for depth, od_wait in EXP09_V2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v5 grid early.")
        ...
        def run_exp10_v5():
            exp_slug = "exp10_v5"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v5: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v5")
            if not best_exp09 or not best_exp09.get("gate_ic"):
                raise RuntimeError("EXP-10 v5 requires a qualified EXP-09 v5 result first.")
            rows = []
            for l2_value in EXP10_L2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v5 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
                        run_seed_validation(FRAME, FEATURE_COLS, bundle["splits"], params, exp_root, label, max_splits=MAX_SPLITS)
                    break
            qualified = [row for row in rows if row["gate_ic"]]
        ...
        def run_exp11_v5():
            exp_slug = "exp11_v5"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v5: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v5")
            if not best_exp10:
                raise RuntimeError("EXP-11 v5 requires EXP-10 v5 results.")
            rows = []
        ...
        def run_exp12_v5():
            exp_slug = "exp12_v5"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v5: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v5")
            if not best_exp11:
                raise RuntimeError("EXP-12 v5 requires EXP-11 v5 results.")
            rows = []
            regime_breakdowns = {}
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v5_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        ...
        This notebook is the rerun-v5 campaign for the sector, event, and collapse experiments.
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v5]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_sector_rerun_v5_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_sector_rerun_v5_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def run_exp13_v5():
            ...
            if "rbi_rate_chg" not in FULL_FRAME.columns:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v5.")
            ...
            write_json(exp_root / "exp13_v5_summary.json", payload)
            write_table(exp_root / "exp13_v5_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v5():
            ...
            if anchor_feature is None:
                raise RuntimeError("An FX anchor feature must be present for EXP-14 v5.")
            ...
            write_json(exp_root / "exp14_v5_summary.json", payload)
            write_table(exp_root / "exp14_v5_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v5():
            ...
            write_json(exp_root / "exp15_v5_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v5():
            ...
            write_json(exp_root / "exp16_v5_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v5():
            ...
            write_json(exp_root / "exp17_v5_summary.json", payload)
        ...
        def run_exp18_v5():
            exp_slug = "exp18_v5"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v5")
            if exp17_state is None:
                exp17_state = run_exp17_v5()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v5_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v5_20260406.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v5_20260406.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v5_20260406.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v5_20260406.ipynb"
