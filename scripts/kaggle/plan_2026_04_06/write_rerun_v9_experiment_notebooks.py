#!/usr/bin/env python3
"""Write brand-new rerun-v9 Kaggle notebooks for EXP-09 through EXP-18."""
...
def _common_runtime(log_prefix: str) -> str:
    return dedent(
        ...
        def run_preflight_checks(bundle, feature_cols):
            ...
            assert not forbidden, f"Precomputed CS transforms leaked into v9 features: {forbidden[:20]}"
...
RATIO_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V9: EXP-09 through EXP-12

        This notebook is the rerun-v9 campaign for the ratio experiments.
    ...
    _code(
        ...
        EXP09_FAVORABLE_REGIMES = list(FAVORABLE_REGIMES)
        SANITY_PROBES = [
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v9]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_ratio_rerun_v9_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_ratio_rerun_v9_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v9", "exp10_v9", "exp09_v9"]:
        ...
        def run_exp09_v9():
            ...
            title = "EXP-09 v9: Raw-Feature Depth x Lookback Policy and Favorable-Regime Routing"
            sanity_rows = []
            sanity_payloads = []
            regime_breakdowns = {}
            ...
            for spec in EXP09_POLICY_GRID:
                ...
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v9 grid early.")
        ...
        def run_exp10_v9():
            exp_slug = "exp10_v9"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v9: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v9")
            if not best_exp09:
                raise RuntimeError("EXP-10 v9 requires EXP-09 v9 results first.")
            if not best_exp09.get("gate_ic") and not ALLOW_BEST_AVAILABLE_CHAIN:
                raise RuntimeError("EXP-10 v9 requires a qualified EXP-09 v9 result when ALLOW_BEST_AVAILABLE_CHAIN is disabled.")
            ...
            for l2_value in EXP10_L2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v9 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
                        run_seed_validation(
                            FRAME,
                            FEATURE_COLS,
        ...
        def run_exp11_v9():
            exp_slug = "exp11_v9"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v9: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v9")
            if not best_exp10:
                raise RuntimeError("EXP-11 v9 requires EXP-10 v9 results.")
        ...
        def run_exp12_v9():
            exp_slug = "exp12_v9"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v9: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v9")
            if not best_exp11:
                raise RuntimeError("EXP-12 v9 requires EXP-11 v9 results.")
            rows = []
            regime_breakdowns = {}
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v9_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V9: EXP-13 through EXP-18

        This notebook is the rerun-v9 campaign for the sector, event, and collapse experiments.
    ...
    _code(
        ...
        TREE_PARAM_OVERRIDES = {}
        EXP13_FAVORABLE_REGIMES = list(FAVORABLE_REGIMES)
        EXP13_FS_CONFIGS = [
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v9]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_sector_rerun_v9_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_sector_rerun_v9_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def run_exp13_v9():
            ...
            if "rbi_rate_chg" not in FULL_FRAME.columns:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v9.")
            ...
            write_json(exp_root / "exp13_v9_summary.json", payload)
            write_table(exp_root / "exp13_v9_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v9():
            ...
            if anchor_feature is None:
                raise RuntimeError("An FX anchor feature must be present for EXP-14 v9.")
            ...
            write_json(exp_root / "exp14_v9_summary.json", payload)
            write_table(exp_root / "exp14_v9_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v9():
            ...
            write_json(exp_root / "exp15_v9_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v9():
            ...
            write_json(exp_root / "exp16_v9_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v9():
            ...
            write_json(exp_root / "exp17_v9_summary.json", payload)
        ...
        def run_exp18_v9():
            exp_slug = "exp18_v9"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v9")
            if exp17_state is None:
                exp17_state = run_exp17_v9()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v9_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v9_20260407.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v9_20260407.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v9_20260407.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v9_20260407.ipynb"
