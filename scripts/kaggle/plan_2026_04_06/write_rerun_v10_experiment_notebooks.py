#!/usr/bin/env python3
"""Write brand-new rerun-v10 Kaggle notebooks for EXP-09 through EXP-18."""
...
def _common_runtime(log_prefix: str) -> str:
    return dedent(
        ...
        ACTIVE_CS_SHARE_MIN = 0.05
        ACTIVE_CS_DATES_MIN = 10
        DROP_REDUNDANT_CS_RANK = True
        FEATURE_AUDIT_STATE = {"kept": [], "quarantined": [], "report": []}
        DEFAULT_TRAIN_LOOKBACK_YEARS = 3
        ...
        FAVORABLE_REGIMES = ("Low-Vol Bull", "Low-Vol Bear", "High-Vol Bear", "Sideways")
        MODEL_BASE_FAMILIES = {"raw", "sensitivity", "sector_dummy", "sentiment"}
        ...
        THEME_INTERACTION_ALLOW = {
            "general": {"fx_x_sens", "fx_x_export", "rate_x_sens", "yc_x_sens", "crude_x_sens", "steel_x_sens", "copper_x_sens", "gold_x_sens", "dxy_x_export"},
            "financials": {"rate_x_sens", "yc_x_sens", "fx_x_sens", "fx_x_export", "dxy_x_export"},
            "lenders": {"rate_x_sens", "yc_x_sens"},
            "other_financials": {"rate_x_sens", "fx_x_sens", "fx_x_export", "dxy_x_export"},
            "it": {"fx_x_sens", "fx_x_export", "dxy_x_export"},
            "capital_goods": {"crude_x_sens", "steel_x_sens", "copper_x_sens", "gold_x_sens"},
        }
        ...
        def build_feature_columns(bundle, extra_exclude=None):
            ...
            for col in candidate_cols:
                ...
                leakage_flag = False
                reason_bits = []
                if family in EXCLUDED_MODEL_FAMILIES:
                    leakage_flag = True
                    reason_bits.append("excluded_transformed_family")
                elif family not in MODEL_BASE_FAMILIES:
                    leakage_flag = True
                    reason_bits.append("excluded_non_base_family")
                ...
                if family in MODEL_BASE_FAMILIES and (active_cs_share < ACTIVE_CS_SHARE_MIN or active_cs_dates < ACTIVE_CS_DATES_MIN):
                    leakage_flag = True
            ...
            kept_by_family = {}
            quarantined_by_family = {}
            if not report_df.empty:
                kept_by_family = report_df.loc[~report_df["quarantined"]].groupby("family").size().to_dict()
                quarantined_by_family = report_df.loc[report_df["quarantined"]].groupby("family").size().to_dict()
            log_kv(
                ...
                quarantined_by_family=quarantined_by_family,
            )
            if quarantined:
        ...
        def add_cross_sectional_interactions(frame, allowed_rhs_features=None):
            working = frame.copy()
            added = []
            source_map = {}
            allowed_rhs_features = set(allowed_rhs_features or [])
            specs = [
                ...
                ("yc_x_sens", ["yield_curve_slope", "yield_curve_slope_ts_z"], "rate_sensitivity_score"),
            ...
            for name, left_candidates, right in specs:
                if name in working.columns or right not in working.columns:
                    continue
                if allowed_rhs_features and right not in allowed_rhs_features:
                    continue
                left = next((candidate for candidate in left_candidates if candidate in working.columns), None)
                if left is None:
                    continue
                working[name] = (
                ...
                added.append(name)
                source_map[name] = {"left": left, "right": right}
            log_kv("Added cross-sectional interactions", count=len(added), features=added, source_map=source_map)
            return working, added


        def prune_feature_cols_for_frame(frame, feature_cols, label, min_non_null_fraction=0.02, min_active_dates=8):
            if frame.empty:
                return list(feature_cols)
            total_rows = max(len(frame), 1)
            total_dates = max(int(frame["date"].nunique()), 1)
            kept = []
            dropped = []
            for col in feature_cols:
                if col not in frame.columns:
                    dropped.append({"feature": col, "family": feature_family(col), "reason": "missing_in_frame"})
                    continue
                series = pd.to_numeric(frame[col], errors="coerce")
                non_null_rows = int(series.notna().sum())
                unique_count = int(series.nunique(dropna=True))
                by_date = series.groupby(frame["date"]).std()
                active_dates = int((by_date.fillna(0.0) > 1e-10).sum())
                family = feature_family(col)
                reasons = []
                if non_null_rows < max(50, int(total_rows * min_non_null_fraction)):
                    reasons.append("too_sparse_for_frame")
                if unique_count <= 1:
                    reasons.append("constant_in_frame")
                if family in {"sector_dummy", "interaction", "sensitivity", "sentiment"} and active_dates < min(min_active_dates, total_dates):
                    reasons.append("low_active_dates_in_frame")
                if family == "raw" and active_dates == 0 and unique_count <= 2:
                    reasons.append("raw_without_cs_activity")
                if reasons:
                    dropped.append({"feature": col, "family": family, "reason": "|".join(reasons), "active_dates": active_dates})
                else:
                    kept.append(col)
            dropped_by_family = {}
            if dropped:
                dropped_by_family = pd.DataFrame(dropped).groupby("family").size().to_dict()
            log_kv(
                "Pruned frame features",
                frame_label=label,
                input_count=len(feature_cols),
                kept_count=len(kept),
                dropped_count=len(dropped),
                dropped_by_family=dropped_by_family,
                dropped_sample=[row["feature"] for row in dropped[:12]],
            )
            return kept


        def themed_feature_cols(feature_cols, theme="general"):
            allowed_interactions = THEME_INTERACTION_ALLOW.get(theme, THEME_INTERACTION_ALLOW["general"])
            selected = []
            for col in feature_cols:
                family = feature_family(col)
                if family == "interaction" and col not in allowed_interactions:
                    continue
                selected.append(col)
            log_kv("Selected themed features", theme=theme, input_count=len(feature_cols), kept_count=len(selected))
            return selected
        ...
        def run_preflight_checks(bundle, feature_cols):
            ...
            forbidden = [
                ...
                if c.endswith("_cs_z") or c.endswith("_cs_rank") or "_sector_z" in c
            ]
            assert not forbidden, f"Precomputed CS transforms leaked into v10 features: {forbidden[:20]}"
        ...
        def build_quality_weights(frame):
            if frame.empty:
                return pd.Series(dtype=float)
            dates = pd.to_datetime(frame["date"], errors="coerce")
            weights = pd.Series(1.0, index=frame.index, dtype=float)
            for start_date, end_date, weight_value, _label in QUALITY_WEIGHT_RULES:
                mask = dates.between(pd.Timestamp(start_date), pd.Timestamp(end_date))
                if bool(mask.any()):
                    weights.loc[mask] = np.minimum(weights.loc[mask], float(weight_value))
            return weights


        def normalize_regime_filter(regimes):
            if not regimes:
                return None
            values = [str(item) for item in regimes if str(item)]
            return tuple(dedupe(values)) if values else None
        ...
        def fit_catboost(train_frame, feature_cols, params, use_early_stopping=True, cat_feature_names=None):
            ...
            sample_weight = None
            if "_sample_weight" in fit_frame.columns:
                sample_weight = pd.to_numeric(fit_frame["_sample_weight"], errors="coerce").fillna(1.0).to_numpy(dtype=float)
            if use_early_stopping and valid_frame is not None and not valid_frame.empty:
                ...
                model.fit(x_fit, y_fit, sample_weight=sample_weight, cat_features=cat_indices if cat_indices else None)
            return model
        ...
        def aggregate_results(window_results):
            ...
            positive_count = sum(1 for ic in all_test_ics if ic > 0)
            ...
            return {
                ...
                "positive_window_count": int(positive_count),
                "positive_window_share": float(positive_count / max(len(window_results), 1)),
        ...
        def regime_breakdown_from_window_results(window_results):
            ...
            summary = (
                ...
                .agg(
                    ...
                    mean_ratio=("ratio", lambda s: float(np.mean([v for v in s if v < 500])) if len(s) else 999.0),
                    positive_windows=("test_ic", lambda s: int((s > 0).sum())),
                    windows=("test_ic", "size"),
                )
            )
        ...
        def run_single_config(
            ...
            allowed_train_regimes=None,
            allowed_test_regimes=None,
        ):
            ...
            allowed_train_regimes = normalize_regime_filter(allowed_train_regimes)
            allowed_test_regimes = normalize_regime_filter(allowed_test_regimes)
            window_results = []
            scored_tests = []
            ...
            for split in selected_splits:
                ...
                effective_window_years = override_window_years if override_window_years is not None else (DEFAULT_TRAIN_LOOKBACK_YEARS if use_default_lookback else None)
                if effective_window_years is not None:
                    train_start = (split_dates["train_end"] - pd.DateOffset(years=int(effective_window_years))).normalize()
                ...
                if allowed_train_regimes is not None and "plan_regime_label" in train_fit.columns:
                    train_fit = train_fit[train_fit["plan_regime_label"].isin(allowed_train_regimes)].copy()
                if allowed_test_regimes is not None and "plan_regime_label" in test_fit.columns:
                    test_fit = test_fit[test_fit["plan_regime_label"].isin(allowed_test_regimes)].copy()
                ...
                if APPLY_QUALITY_WEIGHTS:
                    train_fit["_sample_weight"] = build_quality_weights(train_fit)
                else:
                    train_fit["_sample_weight"] = 1.0
                ...
                log_kv(
                    ...
                    lookback_years=effective_window_years,
                    train_regimes="|".join(allowed_train_regimes) if allowed_train_regimes else "all",
                    test_regimes="|".join(allowed_test_regimes) if allowed_test_regimes else "all",
                    n_train=len(train_fit),
                    n_test=len(test_score),
                    weight_min=round(float(train_fit["_sample_weight"].min()), 4),
                    weight_mean=round(float(train_fit["_sample_weight"].mean()), 4),
                )
        ...
        def run_seed_validation(frame, feature_cols, splits, params, exp_root, label_prefix, seeds=(11, 23, 37, 47, 59), max_splits=None, score_tickers=None, cat_feature_names=None, use_early_stopping=False, use_default_lookback=True):
            rows = []
            for seed in seeds:
                seeded = dict(params)
                ...
                state = run_single_config(
                    ...
                    use_default_lookback=use_default_lookback,
                )
                row = {
                    "seed": int(seed),
...
RATIO_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V10: EXP-09 through EXP-12

        This notebook is the rerun-v10 campaign for the ratio experiments.
        ...
        - the baseline sanity suite is diagnostic, not a hard crash path
        - raw-feature runs default to no early stopping
        - `min_data_in_leaf=1` and `l2_leaf_reg=3` are restored for the raw-feature campaign
        ...
        - feature sets are pruned again per experiment frame before fitting
    ...
    _code(
        ...
        STRICT_SANITY_GATE = False
        ALLOW_BEST_AVAILABLE_CHAIN = True
        RUN_FIVE_SEED_VALIDATION = True
        TREE_PARAM_OVERRIDES = {}
        EXP09_FAVORABLE_REGIMES = ["Low-Vol Bull", "Low-Vol Bear", "High-Vol Bear", "Sideways"]
        SANITY_PROBES = [
            {"label": "sanity_depth4_expand_no_es", "params": {"depth": 4, "od_wait": 20, "l2_leaf_reg": 3, "min_data_in_leaf": 1, "learning_rate": 0.05, "iterations": 400, "boosting_type": "Ordered", "loss_function": "RMSE", "eval_metric": "RMSE", "random_seed": 42}, "use_early_stopping": False, "use_default_lookback": False},
            {"label": "sanity_depth4_roll2_no_es", "params": {"depth": 4, "od_wait": 20, "l2_leaf_reg": 3, "min_data_in_leaf": 1, "learning_rate": 0.05, "iterations": 400, "boosting_type": "Ordered", "loss_function": "RMSE", "eval_metric": "RMSE", "random_seed": 42}, "use_early_stopping": False, "override_window_years": 2},
            {"label": "sanity_depth4_roll3_no_es", "params": {"depth": 4, "od_wait": 20, "l2_leaf_reg": 3, "min_data_in_leaf": 1, "learning_rate": 0.05, "iterations": 400, "boosting_type": "Ordered", "loss_function": "RMSE", "eval_metric": "RMSE", "random_seed": 42}, "use_early_stopping": False, "override_window_years": 3},
        ]

        EXP09_POLICY_GRID = [
            {"label": "depth_3_roll2_no_es", "depth": 3, "od_wait": 20, "override_window_years": 2},
            {"label": "depth_4_roll2_no_es", "depth": 4, "od_wait": 20, "override_window_years": 2},
            {"label": "depth_3_roll3_no_es", "depth": 3, "od_wait": 20, "override_window_years": 3},
            {"label": "depth_4_roll3_no_es", "depth": 4, "od_wait": 20, "override_window_years": 3},
            {"label": "depth_3_roll2_favorable_no_es", "depth": 3, "od_wait": 20, "override_window_years": 2, "allowed_train_regimes": EXP09_FAVORABLE_REGIMES, "allowed_test_regimes": EXP09_FAVORABLE_REGIMES},
            {"label": "depth_4_roll2_favorable_no_es", "depth": 4, "od_wait": 20, "override_window_years": 2, "allowed_train_regimes": EXP09_FAVORABLE_REGIMES, "allowed_test_regimes": EXP09_FAVORABLE_REGIMES},
        ]
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v10]")),
    ...
    _code(
        ...
        FRAME, INTERACTION_FEATURES = add_cross_sectional_interactions(FRAME, allowed_rhs_features=FEATURE_COLS)
        FEATURE_COLS = dedupe([col for col in FEATURE_COLS if col in FRAME.columns] + INTERACTION_FEATURES)
        FEATURE_COLS = themed_feature_cols(FEATURE_COLS, theme="general")
        FEATURE_COLS = prune_feature_cols_for_frame(FRAME, FEATURE_COLS, label="ratio_full_universe")
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_ratio_rerun_v10_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_ratio_rerun_v10_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def candidate_row(config_label, payload):
            ...
            return {
                ...
                "positive_window_share": aggregate["positive_window_share"],
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v10", "exp10_v10", "exp09_v10"]:
        ...
        def sanity_is_workable(best_row):
            if not best_row:
                return False
            if best_row["mean_test_ic"] >= 0.020:
                return True
            return bool(
                best_row["mean_test_ic"] >= 0.010
                and best_row["sign_stability"] >= 0.45
            )
        ...
        def run_exp09_v10():
            ...
            title = "EXP-09 v10: Raw-Feature Depth x Lookback Policy and Favorable-Regime Routing"
            sanity_rows = []
            sanity_payloads = []
            regime_breakdowns = {}
            for probe in SANITY_PROBES:
                sanity_state = run_single_config(
                    ...
                    exp_root=exp_root,
                    max_splits=MAX_SPLITS,
                    use_early_stopping=bool(probe["use_early_stopping"]),
                    use_default_lookback=bool(probe.get("use_default_lookback", True)) if "override_window_years" not in probe else False,
                    override_window_years=probe.get("override_window_years"),
                    allowed_train_regimes=probe.get("allowed_train_regimes"),
                    allowed_test_regimes=probe.get("allowed_test_regimes"),
                )
                sanity_payload = {
                    "config_label": probe["label"],
                    "params": dict(probe["params"]),
                ...
                sanity_payloads.append(sanity_payload)
                row = candidate_row(probe["label"], {"aggregate": sanity_state["aggregate"], "params": dict(probe["params"])})
                row["use_early_stopping"] = bool(probe["use_early_stopping"])
                row["use_default_lookback"] = bool(probe.get("use_default_lookback", True)) if "override_window_years" not in probe else False
                row["override_window_years"] = probe.get("override_window_years")
                row["allowed_train_regimes"] = probe.get("allowed_train_regimes")
                row["allowed_test_regimes"] = probe.get("allowed_test_regimes")
                sanity_rows.append(row)
                regime_breakdowns[probe["label"]] = regime_breakdown_from_window_results(sanity_state["window_results"])
                write_json(exp_root / f"{probe['label']}_results.json", sanity_payload)
            best_sanity = pick_best_result(sanity_rows)
            write_json(exp_root / "sanity_check_results.json", {"best_sanity": best_sanity, "probes": sanity_payloads})
            log_kv(
                "Sanity check result",
                best_probe=(best_sanity or {}).get("config_label"),
                mean_test_ic=(best_sanity or {}).get("mean_test_ic"),
                mean_ratio=(best_sanity or {}).get("mean_ratio"),
            )
            if STRICT_SANITY_GATE and not sanity_is_workable(best_sanity):
                write_text(
                    exp_root / f"{exp_slug}_verdict.txt",
                    "Sanity check suite failed under strict mode. Stopping EXP-09 before the grid.",
                )
                raise RuntimeError("Best sanity probe was not workable under strict mode.")
            if not sanity_is_workable(best_sanity):
                log("Sanity suite did not clear the full IC gate, but it is being treated as diagnostic. Proceeding with the no-early-stopping policy grid.")

            rows = []
            for spec in EXP09_POLICY_GRID:
                params = dict(DEFAULT_TREE_PARAMS)
                params.update(dict(TREE_PARAM_OVERRIDES or {}))
                params.update({"depth": int(spec["depth"]), "od_wait": int(spec["od_wait"]), "l2_leaf_reg": 3, "min_data_in_leaf": 1})
                ...
                state = run_single_config(
                    ...
                    use_default_lookback=bool(spec.get("use_default_lookback", True)) if "override_window_years" not in spec else False,
                    override_window_years=spec.get("override_window_years"),
                    allowed_train_regimes=spec.get("allowed_train_regimes"),
                    allowed_test_regimes=spec.get("allowed_test_regimes"),
                )
                row = candidate_row(label, {"aggregate": state["aggregate"], "params": params})
                row["use_default_lookback"] = bool(spec.get("use_default_lookback", True)) if "override_window_years" not in spec else False
                row["override_window_years"] = spec.get("override_window_years")
                row["use_early_stopping"] = False
                row["allowed_train_regimes"] = spec.get("allowed_train_regimes")
                row["allowed_test_regimes"] = spec.get("allowed_test_regimes")
                write_json(exp_root / f"{label}_results.json", {**row, "window_results": state["window_results"]})
                rows.append(row)
                regime_breakdowns[label] = regime_breakdown_from_window_results(state["window_results"])
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v10 grid early.")
            ...
            payload = write_experiment_summary(
                ...
                rows,
                best,
                extras={"sanity_check": {"best_sanity": best_sanity, "probes": sanity_payloads}, "regime_breakdowns": regime_breakdowns},
            )
        ...
        def run_exp10_v10():
            exp_slug = "exp10_v10"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v10: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v10")
            if not best_exp09:
                raise RuntimeError("EXP-10 v10 requires EXP-09 v10 results first.")
            if not best_exp09.get("gate_ic") and not ALLOW_BEST_AVAILABLE_CHAIN:
                raise RuntimeError("EXP-10 v10 requires a qualified EXP-09 v10 result when ALLOW_BEST_AVAILABLE_CHAIN is disabled.")
            lookback_policy = bool(best_exp09.get("use_default_lookback", True))
            override_years = best_exp09.get("override_window_years")
            rows = []
            for l2_value in EXP10_L2_GRID:
                ...
                state = run_single_config(
                    ...
                    max_splits=MAX_SPLITS,
                    use_early_stopping=False,
                    use_default_lookback=lookback_policy if override_years is None else False,
                    override_window_years=override_years,
                    allowed_train_regimes=best_exp09.get("allowed_train_regimes"),
                    allowed_test_regimes=best_exp09.get("allowed_test_regimes"),
                )
                row = candidate_row(label, {"aggregate": state["aggregate"], "params": params})
                row["l2_leaf_reg"] = float(l2_value)
                row["use_default_lookback"] = lookback_policy
                row["override_window_years"] = override_years
                row["allowed_train_regimes"] = best_exp09.get("allowed_train_regimes")
                row["allowed_test_regimes"] = best_exp09.get("allowed_test_regimes")
                write_json(exp_root / f"{label}_results.json", {**row, "window_results": state["window_results"]})
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v10 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
                        run_seed_validation(
                            FRAME,
                            FEATURE_COLS,
        ...
        def run_exp11_v10():
            exp_slug = "exp11_v10"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v10: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v10")
            if not best_exp10:
                raise RuntimeError("EXP-11 v10 requires EXP-10 v10 results.")
            lookback_policy = bool(best_exp10.get("use_default_lookback", True))
            override_years = best_exp10.get("override_window_years")
            rows = []
            for config in EXP11_CONFIGS:
                ...
                state = run_single_config(
                    ...
                    use_default_lookback=lookback_policy if override_years is None else False,
                    override_window_years=override_years,
                    allowed_train_regimes=best_exp10.get("allowed_train_regimes"),
                    allowed_test_regimes=best_exp10.get("allowed_test_regimes"),
                )
                row = candidate_row(label, {"aggregate": state["aggregate"], "params": params})
                ...
                row["use_default_lookback"] = lookback_policy
                row["override_window_years"] = override_years
                row["allowed_train_regimes"] = best_exp10.get("allowed_train_regimes")
                row["allowed_test_regimes"] = best_exp10.get("allowed_test_regimes")
                write_json(exp_root / f"{label}_results.json", {**row, "window_results": state["window_results"]})
                rows.append(row)
        ...
        def run_exp12_v10():
            exp_slug = "exp12_v10"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v10: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v10")
            if not best_exp11:
                raise RuntimeError("EXP-12 v10 requires EXP-11 v10 results.")
            rows = []
            regime_breakdowns = {}
            for years in EXP12_WINDOW_YEARS:
                ...
                state = run_single_config(
                    ...
                    allowed_train_regimes=best_exp11.get("allowed_train_regimes"),
                    allowed_test_regimes=best_exp11.get("allowed_test_regimes"),
                )
                row = candidate_row(label, {"aggregate": state["aggregate"], "params": params})
                row["window_years"] = int(years)
                row["use_default_lookback"] = False
                row["allowed_train_regimes"] = best_exp11.get("allowed_train_regimes")
                row["allowed_test_regimes"] = best_exp11.get("allowed_test_regimes")
                write_json(exp_root / f"{label}_results.json", {**row, "window_results": state["window_results"]})
                rows.append(row)
                regime_table = (
        ...
        RUNNERS = {
            "EXP-09": run_exp09_v10,
            "EXP-10": run_exp10_v10,
            "EXP-11": run_exp11_v10,
            "EXP-12": run_exp12_v10,
        }
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v10_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V10: EXP-13 through EXP-18

        This notebook is the rerun-v10 campaign for the sector, event, and collapse experiments.
    ...
    _code(
        ...
        USE_EARLY_STOPPING = False
        RUN_UNIVERSAL_ON_FS = False
        TREE_PARAM_OVERRIDES = {}
        EXP13_FAVORABLE_REGIMES = ["Low-Vol Bull", "Low-Vol Bear", "High-Vol Bear", "Sideways"]
        EXP13_FS_CONFIGS = [
            {"label": "fs_l2_4_no_es", "depth": 3, "l2_leaf_reg": 4},
            {"label": "fs_l2_8_no_es", "depth": 3, "l2_leaf_reg": 8},
            {"label": "fs_l2_4_favorable_no_es", "depth": 3, "l2_leaf_reg": 4, "allowed_train_regimes": EXP13_FAVORABLE_REGIMES, "allowed_test_regimes": EXP13_FAVORABLE_REGIMES},
        ]
        EXP13_LENDERS_CONFIGS = [
            {"label": "lenders_l2_8_no_es", "depth": 2, "l2_leaf_reg": 8, "iterations": 250},
            {"label": "lenders_l2_12_no_es", "depth": 2, "l2_leaf_reg": 12, "iterations": 250},
            {"label": "lenders_l2_8_favorable_no_es", "depth": 2, "l2_leaf_reg": 8, "iterations": 250, "allowed_train_regimes": EXP13_FAVORABLE_REGIMES, "allowed_test_regimes": EXP13_FAVORABLE_REGIMES},
        ]
        EXP13_OTHER_FIN_CONFIGS = [
            {"label": "other_fin_l2_6_no_es", "depth": 2, "l2_leaf_reg": 6, "iterations": 250},
            {"label": "other_fin_l2_10_no_es", "depth": 2, "l2_leaf_reg": 10, "iterations": 250},
            {"label": "other_fin_l2_6_favorable_no_es", "depth": 2, "l2_leaf_reg": 6, "iterations": 250, "allowed_train_regimes": EXP13_FAVORABLE_REGIMES, "allowed_test_regimes": EXP13_FAVORABLE_REGIMES},
        ]
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v10]")),
    ...
    _code(
        ...
        FULL_FRAME, INTERACTION_FEATURES = add_cross_sectional_interactions(FULL_FRAME, allowed_rhs_features=BASE_FEATURE_COLS)
        BASE_FEATURE_COLS = dedupe([col for col in BASE_FEATURE_COLS if col in FULL_FRAME.columns] + INTERACTION_FEATURES)
        BASE_FEATURE_COLS = themed_feature_cols(BASE_FEATURE_COLS, theme="general")
        BASE_FEATURE_COLS = prune_feature_cols_for_frame(FULL_FRAME, BASE_FEATURE_COLS, label="sector_full_universe")
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_sector_rerun_v10_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_sector_rerun_v10_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def prepare_feature_set(frame, theme, label):
            themed = themed_feature_cols(BASE_FEATURE_COLS, theme=theme)
            return prune_feature_cols_for_frame(frame, themed, label=label)


        def tuned_params_for_universe(base_params, ticker_count, explicit_keys=None):
            explicit_keys = set(explicit_keys or [])
            params = dict(base_params)
            if ticker_count <= 25:
                if "depth" not in explicit_keys:
                    params["depth"] = min(int(params.get("depth", 3)), 2)
                if "l2_leaf_reg" not in explicit_keys:
                    params["l2_leaf_reg"] = max(float(params.get("l2_leaf_reg", 3)), 8.0)
                if "iterations" not in explicit_keys:
                    params["iterations"] = min(int(params.get("iterations", 400)), 250)
            elif ticker_count <= 75:
                if "depth" not in explicit_keys:
                    params["depth"] = min(int(params.get("depth", 3)), 3)
                if "l2_leaf_reg" not in explicit_keys:
                    params["l2_leaf_reg"] = max(float(params.get("l2_leaf_reg", 3)), 4.0)
            return params


        def run_config_sweep(frame, feature_cols, params_list, label_prefix, exp_root, max_splits=None):
            rows = []
            states = {}
            for spec in params_list:
                params = tuned_params_for_universe({**BASE_PARAMS, **spec}, int(frame["ticker"].nunique()), explicit_keys=set(spec.keys()))
                label = str(spec["label"])
                state = run_single_config(
                    frame,
                    feature_cols,
                ...
                row = candidate_summary(label, state, params)
                row["regime_breakdown"] = regime_breakdown_from_window_results(state["window_results"])
                row["allowed_train_regimes"] = spec.get("allowed_train_regimes")
                row["allowed_test_regimes"] = spec.get("allowed_test_regimes")
                rows.append(row)
                states[label] = {"row": row, "state": state, "params": params}
            best = pick_best_result(rows) or (rows[0] if rows else None)
            return rows, states, best
        ...
        def candidate_summary(label, state, params, extra=None):
            row = {
                ...
                "positive_window_count": state["aggregate"]["positive_window_count"],
                "positive_window_share": state["aggregate"]["positive_window_share"],
        ...
        def explain_it_feature_importance(it_frame, feature_cols, params, windows=(5, 10, 15)):
            ...
            for window_id in windows:
                ...
                model = fit_catboost(train_window, feature_cols, params, use_early_stopping=USE_EARLY_STOPPING)
                method = "gain"
                if shap is not None:
                    try:
        ...
        def run_exp13_v10():
            ...
            if "rbi_rate_chg" not in FULL_FRAME.columns:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v10.")
            sector_frame = subset_frame(FULL_FRAME, fs_tickers)
            fs_feature_cols = prepare_feature_set(sector_frame, "financials", "exp13_full_fs")
            fs_rows, fs_states, best_fs = run_config_sweep(sector_frame, fs_feature_cols, EXP13_FS_CONFIGS, "fs", exp_root, max_splits=MAX_SPLITS)
            best_fs = pick_best_result(fs_rows) or fs_rows[0]
            best_fs_state = fs_states[best_fs["config_label"]]["state"]
            best_fs_params = fs_states[best_fs["config_label"]]["params"]
            universal_summary = {"status": "SKIPPED", "reason": "disabled_by_default"}
            universal_state = None
            if RUN_UNIVERSAL_ON_FS:
                ...
                universal_summary = candidate_summary("universal_on_fs", universal_state, best_fs_params, {"model_type": "full_universe_fit_fs_subset_eval_aligned"})
            subuniverse_rows = []
            subuniverse_details = {}
            for label in ["lenders", "banks", "nbfcs", "insurance", "other_financials"]:
                ...
                theme = "lenders" if label in {"lenders", "banks", "nbfcs"} else "other_financials"
                sub_features = prepare_feature_set(sub_frame, theme, f"exp13_{label}")
                config_list = EXP13_LENDERS_CONFIGS if label == "lenders" else EXP13_OTHER_FIN_CONFIGS if label == "other_financials" else [{"label": f"{label}_depth2_no_es", "depth": 2, "l2_leaf_reg": 8, "iterations": 250}]
                sweep_rows, sweep_states, sweep_best = run_config_sweep(sub_frame, sub_features, config_list, label, exp_root, max_splits=MAX_SPLITS)
                sweep_best = sweep_best or sweep_rows[0]
                subuniverse_rows.append({"sub_universe": label, "status": "RUN", "ticker_count": len(tickers), "mean_test_ic": sweep_best["mean_test_ic"], "mean_ratio": sweep_best["mean_ratio"], "positive_window_share": sweep_best["positive_window_share"], "best_config": sweep_best["config_label"]})
                subuniverse_details[label] = {"rows": sweep_rows, "best": sweep_best}
            rate_breakdown = rate_regime_breakdown(best_fs_state["scored_tests"])
            payload = {
                "exp_slug": exp_slug,
                "sector_ticker_count": len(fs_tickers),
                "fs_config_sweep": fs_rows,
                "sector_only": candidate_summary(best_fs["config_label"], best_fs_state, best_fs_params, {"model_type": "sector_only"}),
                "universal_on_fs": universal_summary,
                "rate_regime_breakdown": rate_breakdown,
                "sub_universes": subuniverse_rows,
                "sub_universe_details": subuniverse_details,
                "pass_sector_ic": bool(best_fs_state["aggregate"]["mean_test_ic"] > 0.015),
                "pass_vs_universal": bool(best_fs_state["aggregate"]["mean_test_ic"] > (universal_state["aggregate"]["mean_test_ic"] if universal_state is not None else -999.0)),
            }
            write_json(exp_root / "exp13_v10_summary.json", payload)
            write_table(exp_root / "exp13_v10_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v10():
            ...
            if anchor_feature is None:
                raise RuntimeError("An FX anchor feature must be present for EXP-14 v10.")
            it_frame = subset_frame(FULL_FRAME, tickers)
            it_features = prepare_feature_set(it_frame, "it", "exp14_it")
            it_params = tuned_params_for_universe(BASE_PARAMS, len(tickers))
            sector_state = run_single_config(it_frame, it_features, bundle["splits"], it_params, "it_sector_only", exp_root, max_splits=MAX_SPLITS, use_early_stopping=USE_EARLY_STOPPING)
            standalone_ic = standalone_feature_ic(it_frame, anchor_feature if anchor_feature in it_frame.columns else it_features[0])
            ...
            importance_rows = explain_it_feature_importance(it_frame, it_features, it_params)
            payload = {
                "exp_slug": exp_slug,
                "ticker_count": len(tickers),
                "fx_anchor_feature": anchor_feature,
                "standalone_inr_ic": standalone_ic,
                "sector_only": candidate_summary("it_sector_only", sector_state, it_params),
            ...
            write_json(exp_root / "exp14_v10_summary.json", payload)
            write_table(exp_root / "exp14_v10_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v10():
            ...
            cg_features = prepare_feature_set(cg_frame, "capital_goods", "exp15_capital_goods")
            cg_params = tuned_params_for_universe(BASE_PARAMS, len(tickers))
            base_state = run_single_config(cg_frame, cg_features, bundle["splits"], cg_params, "capital_goods_base", exp_root, max_splits=MAX_SPLITS, use_early_stopping=USE_EARLY_STOPPING)
            augmented_frame, extra_features = add_capital_goods_interaction(cg_frame)
            augmented_features = prune_feature_cols_for_frame(
                augmented_frame,
                dedupe(cg_features + [col for col in extra_features if col not in cg_features]),
                label="exp15_capital_goods_augmented",
            )
            interaction_state = run_single_config(augmented_frame, augmented_features, bundle["splits"], cg_params, "capital_goods_interaction", exp_root, max_splits=MAX_SPLITS, use_early_stopping=USE_EARLY_STOPPING)
            universal_state = run_single_config(
                FULL_FRAME,
                cg_features,
                bundle["splits"],
            ...
            payload = {
                "exp_slug": exp_slug,
                "ticker_count": len(tickers),
                "base_model": candidate_summary("capital_goods_base", base_state, cg_params),
                "interaction_model": candidate_summary("capital_goods_interaction", interaction_state, cg_params, {"extra_features": extra_features}),
                "universal_on_cg": candidate_summary("universal_on_cg", universal_state, cg_params),
            ...
            write_json(exp_root / "exp15_v10_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v10():
            ...
            blend_base_features = prepare_feature_set(blend_frame, "general", "exp16_blend")
            blend_features = dedupe(blend_base_features + ["sector_id"])
            blend_params = tuned_params_for_universe(BASE_PARAMS, len(blend_tickers))
            selected_splits = [split for split in bundle["splits"] if 8 <= int(split["window_id"]) <= 15]
            base_state = run_single_config(blend_frame, blend_base_features, selected_splits, blend_params, "blend_without_sector_id", exp_root, max_splits=MAX_SPLITS, use_early_stopping=USE_EARLY_STOPPING)
            blend_state = run_single_config(blend_frame, blend_features, selected_splits, blend_params, "blend_with_sector_id", exp_root, max_splits=MAX_SPLITS, cat_feature_names=["sector_id"], use_early_stopping=USE_EARLY_STOPPING)
            payload = {
                "exp_slug": exp_slug,
                "ticker_count": len(blend_tickers),
                "windows_tested": [int(split["window_id"]) for split in selected_splits],
                "baseline_without_sector_id": candidate_summary("blend_without_sector_id", base_state, blend_params),
                "blend_with_sector_id": candidate_summary("blend_with_sector_id", blend_state, blend_params),
                "exp07_baseline_ic": EXP07_BASELINE_IC,
                "improvement_vs_exp07": float(blend_state["aggregate"]["mean_test_ic"] - EXP07_BASELINE_IC),
            }
            write_json(exp_root / "exp16_v10_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v10():
            ...
            write_json(exp_root / "exp17_v10_summary.json", payload)
        ...
        def run_exp18_v10():
            exp_slug = "exp18_v10"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v10")
            if exp17_state is None:
                exp17_state = run_exp17_v10()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v10_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload


        RUNNERS = {
            "EXP-13": run_exp13_v10,
            "EXP-14": run_exp14_v10,
            "EXP-15": run_exp15_v10,
            "EXP-16": run_exp16_v10,
            "EXP-17": run_exp17_v10,
            "EXP-18": run_exp18_v10,
        }
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v10_20260407.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v10_20260407.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v10_20260407.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v10_20260407.ipynb"
