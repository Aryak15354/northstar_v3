#!/usr/bin/env python3
"""Write brand-new rerun-v2 Kaggle notebooks for EXP-09 through EXP-18."""
...
def _common_runtime(log_prefix: str) -> str:
    return dedent(
        ...
        def build_feature_columns(bundle, extra_exclude=None):
            ...
            log_kv("Built feature column list", feature_count=len(feature_cols))
            return feature_cols
        ...
        def compute_window_ic(scored_df, pred_col="pred", target_col=TARGET_COL):
            ...
            nan_count = 0
            if scored_df.empty:
                return {"mean_ic": 0.0, "date_ics": [], "nan_flag": True, "nan_dates": 0}
        ...
        def run_single_config(
            ...
            for split in selected_splits:
                ...
                fit_window = reference_frame[
                    reference_frame["date"].between(train_start, split_dates["test_end"])
                ].copy()
                fit_window = normalize_cs(fit_window, feature_cols, skip_cols=cat_feature_names)
                train_fit = fit_window[fit_window["date"].between(train_start, split_dates["train_end"])].copy()
                test_fit = fit_window[fit_window["date"].between(split_dates["test_start"], split_dates["test_end"])].copy()
                if score_ticker_set:
                    train_score = train_fit[train_fit["ticker"].isin(score_ticker_set)].copy()
                    test_score = test_fit[test_fit["ticker"].isin(score_ticker_set)].copy()
                else:
                ...
                result = {
                    ...
                    "n_test": int(len(test_score)),
                    "regime": regime,
                    "nan_flag": bool(train_metrics["nan_flag"] or test_metrics["nan_flag"]),
                    "is_leak_window": bool(window_id in LEAK_WINDOW_IDS),
                }
                window_results.append(result)
                log_kv(
...
RATIO_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V2: EXP-09 through EXP-12

        This notebook is the rerun-v2 campaign for the ratio experiments.
        ...
        - `min_data_in_leaf=1` and `l2_leaf_reg=3` are restored for EXP-09 v2
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v2]")),
    _md("## EXP-09 through EXP-12"),
    _code(
        """
        FEATURE_COLS = build_feature_columns(bundle)
        FRAME = build_model_frame(bundle, FEATURE_COLS)
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v2", "exp10_v2", "exp09_v2"]:
        ...
        def run_exp09_v2():
            ...
            title = "EXP-09 v2: CatBoost Depth x Early Stopping"
            sanity_params = {
                "depth": 6,
                "od_wait": 30,
                "l2_leaf_reg": 3,
            ...
            if FORCE_STOP_ON_SANITY_FAIL and sanity_state["aggregate"]["mean_test_ic"] < 0.020:
                write_text(
                    ...
                    "Sanity check failed: mean_test_ic below 0.020. Stopping EXP-09 v2 before the grid.",
                )
                raise RuntimeError("Sanity IC < 0.020. Dataset or feature pipeline issue must be resolved before EXP-09 v2.")
            ...
            for depth, od_wait in EXP09_V2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v2 grid early.")
        ...
        def run_exp10_v2():
            exp_slug = "exp10_v2"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v2: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v2")
            if not best_exp09 or not best_exp09.get("gate_ic"):
                raise RuntimeError("EXP-10 v2 requires a qualified EXP-09 v2 result first.")
            rows = []
            for l2_value in EXP10_L2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v2 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
                        run_seed_validation(FRAME, FEATURE_COLS, bundle["splits"], params, exp_root, label, max_splits=MAX_SPLITS)
                    break
            qualified = [row for row in rows if row["gate_ic"]]
        ...
        def run_exp11_v2():
            exp_slug = "exp11_v2"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v2: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v2")
            if not best_exp10:
                raise RuntimeError("EXP-11 v2 requires EXP-10 v2 results.")
            rows = []
        ...
        def run_exp12_v2():
            exp_slug = "exp12_v2"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v2: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v2")
            if not best_exp11:
                raise RuntimeError("EXP-12 v2 requires EXP-11 v2 results.")
            rows = []
            regime_breakdowns = {}
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v2_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V2: EXP-13 through EXP-18

        This notebook is the rerun-v2 campaign for the sector, event, and collapse experiments.
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v2]")),
    _md("## EXP-13 through EXP-18"),
    _code(
        """
        BASE_FEATURE_COLS = build_feature_columns(bundle)
        FULL_FRAME = build_model_frame(bundle, BASE_FEATURE_COLS)
        ...
        def finance_universes():
            ...
            include_mask = (
                ...
                | subsector_lower.str.contains(r"\\bbank|insurance|nbfc|finance|asset management|housing finance|gold loan", regex=True, na=False)
            )
            exclude_mask = subsector_lower.str.contains("banking software|bfsi it|it services", na=False)
            ...
            log_kv("Financial Services universe", full_fs=len(full_fs), banks=len(banks), nbfcs=len(nbfcs), insurance=len(insurance))
            return {"full_fs": full_fs, "banks": banks, "nbfcs": nbfcs, "insurance": insurance}
        ...
        def explain_it_feature_importance(it_frame, feature_cols, params, windows=(5, 10, 15)):
            ...
            for window_id in windows:
                ...
                if train_window.empty or test_window.empty:
                    continue
                normalized = normalize_cs(pd.concat([train_window, test_window], ignore_index=True), feature_cols)
                train_window = normalized[normalized["date"].between(split_dates["train_start"], split_dates["train_end"])].copy()
                test_window = normalized[normalized["date"].between(split_dates["test_start"], split_dates["test_end"])].copy()
                ...
                anchor_rows = ranking[ranking["feature"] == "inrusd_4w_return"]
                ...
                rows.append(
                    {
                        ...
                        "method": method,
                        "anchor_feature": "inrusd_4w_return",
        ...
        def exp17_hypotheses(long_df):
            ...
            for hypothesis, factor, event_id, threshold_value in [
                ...
                ("H3_E020", "inrusd_4w_return", "E020", 0.030),
            ]:
        ...
        def run_exp13_v2():
            ...
            if "rbi_rate_chg" not in BASE_FEATURE_COLS:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v2.")
            ...
            universal_state = run_single_config(FULL_FRAME, BASE_FEATURE_COLS, bundle["splits"], BASE_PARAMS, "universal_on_fs", exp_root, max_splits=MAX_SPLITS, fit_frame=FULL_FRAME, score_tickers=fs_tickers)
            subuniverse_rows = []
            for label in ["banks", "nbfcs", "insurance"]:
            ...
            payload = {
                ...
                "universal_on_fs": candidate_summary("universal_on_fs", universal_state, BASE_PARAMS, {"model_type": "universal_on_subset"}),
            ...
            write_json(exp_root / "exp13_v2_summary.json", payload)
            write_table(exp_root / "exp13_v2_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v2():
            ...
            exp_root = experiment_dir(exp_slug)
            tickers = it_universe()
            if "inrusd_4w_return" not in BASE_FEATURE_COLS:
                raise RuntimeError("inrusd_4w_return must be present for EXP-14 v2.")
            ...
            standalone_ic = standalone_feature_ic(it_frame, "inrusd_4w_return")
            ...
            write_json(exp_root / "exp14_v2_summary.json", payload)
            write_table(exp_root / "exp14_v2_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v2():
            ...
            universal_state = run_single_config(FULL_FRAME, BASE_FEATURE_COLS, bundle["splits"], BASE_PARAMS, "universal_on_cg", exp_root, max_splits=MAX_SPLITS, fit_frame=FULL_FRAME, score_tickers=tickers)
            payload = {
                "exp_slug": exp_slug,
                "ticker_count": len(tickers),
            ...
            write_json(exp_root / "exp15_v2_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v2():
            ...
            write_json(exp_root / "exp16_v2_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v2():
            ...
            write_json(exp_root / "exp17_v2_summary.json", payload)
        ...
        def run_exp18_v2():
            exp_slug = "exp18_v2"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v2")
            if exp17_state is None:
                exp17_state = run_exp17_v2()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v2_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v2_20260406.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v2_20260406.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v2_20260406.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v2_20260406.ipynb"
