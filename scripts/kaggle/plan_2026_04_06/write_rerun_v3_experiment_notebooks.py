#!/usr/bin/env python3
"""Write brand-new rerun-v3 Kaggle notebooks for EXP-09 through EXP-18."""
...
RATIO_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V3: EXP-09 through EXP-12

        This notebook is the rerun-v3 campaign for the ratio experiments.
        ...
        - `min_data_in_leaf=1` and `l2_leaf_reg=3` are restored for EXP-09 v3
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v3]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_ratio_rerun_v3_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_ratio_rerun_v3_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v3", "exp10_v3", "exp09_v3"]:
        ...
        def run_exp09_v3():
            ...
            title = "EXP-09 v3: CatBoost Depth x Early Stopping"
            sanity_params = {
                "depth": 6,
                "od_wait": 30,
                "l2_leaf_reg": 3,
            ...
            if FORCE_STOP_ON_SANITY_FAIL and sanity_state["aggregate"]["mean_test_ic"] < 0.020:
                write_text(
                    ...
                    "Sanity check failed: mean_test_ic below 0.020. Stopping EXP-09 v3 before the grid.",
                )
                raise RuntimeError("Sanity IC < 0.020. Dataset or feature pipeline issue must be resolved before EXP-09 v3.")
            ...
            for depth, od_wait in EXP09_V2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v3 grid early.")
        ...
        def run_exp10_v3():
            exp_slug = "exp10_v3"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v3: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v3")
            if not best_exp09 or not best_exp09.get("gate_ic"):
                raise RuntimeError("EXP-10 v3 requires a qualified EXP-09 v3 result first.")
            rows = []
            for l2_value in EXP10_L2_GRID:
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v3 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
                        run_seed_validation(FRAME, FEATURE_COLS, bundle["splits"], params, exp_root, label, max_splits=MAX_SPLITS)
                    break
            qualified = [row for row in rows if row["gate_ic"]]
        ...
        def run_exp11_v3():
            exp_slug = "exp11_v3"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v3: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v3")
            if not best_exp10:
                raise RuntimeError("EXP-11 v3 requires EXP-10 v3 results.")
            rows = []
        ...
        def run_exp12_v3():
            exp_slug = "exp12_v3"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v3: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v3")
            if not best_exp11:
                raise RuntimeError("EXP-12 v3 requires EXP-11 v3 results.")
            rows = []
            regime_breakdowns = {}
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v3_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V3: EXP-13 through EXP-18

        This notebook is the rerun-v3 campaign for the sector, event, and collapse experiments.
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v3]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_sector_rerun_v3_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_sector_rerun_v3_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def run_exp13_v3():
            ...
            if "rbi_rate_chg" not in BASE_FEATURE_COLS:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v3.")
            ...
            payload = {
                ...
                "universal_on_fs": candidate_summary("universal_on_fs", universal_state, BASE_PARAMS, {"model_type": "full_universe_fit_fs_subset_eval"}),
            ...
            write_json(exp_root / "exp13_v3_summary.json", payload)
            write_table(exp_root / "exp13_v3_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v3():
            ...
            if "inrusd_4w_return" not in BASE_FEATURE_COLS:
                raise RuntimeError("inrusd_4w_return must be present for EXP-14 v3.")
            ...
            write_json(exp_root / "exp14_v3_summary.json", payload)
            write_table(exp_root / "exp14_v3_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v3():
            ...
            write_json(exp_root / "exp15_v3_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v3():
            ...
            write_json(exp_root / "exp16_v3_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v3():
            ...
            write_json(exp_root / "exp17_v3_summary.json", payload)
        ...
        def run_exp18_v3():
            exp_slug = "exp18_v3"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v3")
            if exp17_state is None:
                exp17_state = run_exp17_v3()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v3_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v3_20260406.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v3_20260406.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v3_20260406.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v3_20260406.ipynb"
