#!/usr/bin/env python3
"""Write brand-new rerun-v7 Kaggle notebooks for EXP-09 through EXP-18."""
...
def _common_runtime(log_prefix: str) -> str:
    return dedent(
        ...
        def run_preflight_checks(bundle, feature_cols):
            ...
            assert not forbidden, f"Precomputed CS transforms leaked into v7 features: {forbidden[:20]}"
...
RATIO_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V5: EXP-09 through EXP-12

        This notebook is the rerun-v7 campaign for the ratio experiments.
    ...
    _code(
        ...
        SANITY_PROBES = [
            ...
            {"label": "sanity_depth4_roll3_no_es", "params": {"depth": 4, "od_wait": 20, "l2_leaf_reg": 3, "min_data_in_leaf": 1, "learning_rate": 0.05, "iterations": 400, "boosting_type": "Ordered", "loss_function": "RMSE", "eval_metric": "RMSE", "random_seed": 42}, "use_early_stopping": False, "use_default_lookback": True},
        ]

        EXP09_POLICY_GRID = [
            {"label": "depth_3_expand_no_es", "depth": 3, "od_wait": 20, "use_default_lookback": False},
            {"label": "depth_4_expand_no_es", "depth": 4, "od_wait": 20, "use_default_lookback": False},
            {"label": "depth_3_roll3_no_es", "depth": 3, "od_wait": 20, "use_default_lookback": True},
            {"label": "depth_4_roll3_no_es", "depth": 4, "od_wait": 20, "use_default_lookback": True},
            {"label": "depth_5_roll3_no_es", "depth": 5, "od_wait": 20, "use_default_lookback": True},
        ]
    ...
    _md("## Runtime"),
    _code(_common_runtime("[ratio-rerun-v7]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_ratio_rerun_v7_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_ratio_rerun_v7_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def resolve_best_params():
            ...
            for exp_slug in ["exp11_v7", "exp10_v7", "exp09_v7"]:
        ...
        def run_exp09_v7():
            ...
            title = "EXP-09 v7: Raw-Feature Depth x Lookback Policy"
            sanity_rows = []
            sanity_payloads = []
            for probe in SANITY_PROBES:
                sanity_state = run_single_config(
                    ...
                    use_default_lookback=bool(probe.get("use_default_lookback", True)),
                )
                sanity_payload = {
                    "config_label": probe["label"],
                    "params": dict(probe["params"]),
                ...
                row["use_default_lookback"] = bool(probe.get("use_default_lookback", True))
            ...
            for spec in EXP09_POLICY_GRID:
                ...
                state = run_single_config(
                    ...
                    use_default_lookback=bool(spec.get("use_default_lookback", True)),
                )
                row = candidate_row(label, {"aggregate": state["aggregate"], "params": params})
                row["use_default_lookback"] = bool(spec.get("use_default_lookback", True))
                row["use_early_stopping"] = False
                write_json(exp_root / f"{label}_results.json", {**row, "window_results": state["window_results"]})
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-09 v7 grid early.")
            ...
            payload = write_experiment_summary(
                ...
                rows,
                best,
                extras={"sanity_check": {"best_sanity": best_sanity, "probes": sanity_payloads}},
            )
        ...
        def run_exp10_v7():
            exp_slug = "exp10_v7"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-10 v7: L2 Regularization Sweep"
            best_exp09 = load_previous_best("exp09_v7")
            if not best_exp09:
                raise RuntimeError("EXP-10 v7 requires EXP-09 v7 results first.")
            if not best_exp09.get("gate_ic") and not ALLOW_BEST_AVAILABLE_CHAIN:
                raise RuntimeError("EXP-10 v7 requires a qualified EXP-09 v7 result when ALLOW_BEST_AVAILABLE_CHAIN is disabled.")
            ...
            for l2_value in EXP10_L2_GRID:
                ...
                state = run_single_config(
                    ...
                    max_splits=MAX_SPLITS,
                    use_early_stopping=False,
                    use_default_lookback=lookback_policy,
                )
                row = candidate_row(label, {"aggregate": state["aggregate"], "params": params})
                ...
                rows.append(row)
                if row["verdict_a"]:
                    log(f"Verdict A achieved at {label}. Stopping EXP-10 v7 sweep early.")
                    if RUN_FIVE_SEED_VALIDATION:
        ...
        def run_exp11_v7():
            exp_slug = "exp11_v7"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-11 v7: Ordered vs Plain Boosting"
            best_exp10 = load_previous_best("exp10_v7")
            if not best_exp10:
                raise RuntimeError("EXP-11 v7 requires EXP-10 v7 results.")
        ...
        def run_exp12_v7():
            exp_slug = "exp12_v7"
            exp_root = experiment_dir(exp_slug)
            title = "EXP-12 v7: Training Window Length Extension"
            best_exp11 = load_previous_best("exp11_v7")
            if not best_exp11:
                raise RuntimeError("EXP-12 v7 requires EXP-11 v7 results.")
            rows = []
            regime_breakdowns = {}
    ...
    _code(
        """
        existing = sorted(str(path) for path in OUTPUT_BASE.glob("northstar_exp*_v7_*"))
...
SECTOR_V2_NOTEBOOK = [
    _md(
        """
        # NORTHSTAR V3 Rerun V5: EXP-13 through EXP-18

        This notebook is the rerun-v7 campaign for the sector, event, and collapse experiments.
    ...
    _md("## Runtime"),
    _code(_common_runtime("[sector-regime-rerun-v7]")),
    ...
    _code(
        ...
        if FEATURE_AUDIT_STATE.get("report"):
            write_table(OUTPUT_BASE / "northstar_sector_rerun_v7_feature_audit.csv", pd.DataFrame(FEATURE_AUDIT_STATE["report"]))
            write_json(OUTPUT_BASE / "northstar_sector_rerun_v7_feature_audit.json", FEATURE_AUDIT_STATE)
            log_feature_audit()
        ...
        def tuned_params_for_universe(base_params, ticker_count):
            ...
            if ticker_count <= 25:
                ...
                params["l2_leaf_reg"] = max(float(params.get("l2_leaf_reg", 3)), 6.0)
                params["iterations"] = min(int(params.get("iterations", 400)), 300)
        ...
        def run_exp13_v7():
            ...
            if "rbi_rate_chg" not in FULL_FRAME.columns:
                raise RuntimeError("rbi_rate_chg must be present for EXP-13 v7.")
            sector_frame = subset_frame(FULL_FRAME, fs_tickers)
            fs_feature_cols = prepare_feature_set(sector_frame, "financials", "exp13_full_fs")
            fs_rows = []
            fs_states = {}
            for spec in EXP13_FS_CONFIGS:
                params = tuned_params_for_universe({**BASE_PARAMS, **spec}, len(fs_tickers))
                state = run_single_config(
                    sector_frame,
                    fs_feature_cols,
                    bundle["splits"],
                ...
                row = candidate_summary(spec["label"], state, params, {"model_type": "sector_only"})
                fs_rows.append(row)
                fs_states[spec["label"]] = {"row": row, "state": state, "params": params}
            ...
            for label in ["lenders", "banks", "nbfcs", "insurance", "other_financials"]:
                ...
                sub_params = tuned_params_for_universe(best_fs_params, len(tickers))
                state = run_single_config(
                    sub_frame,
                    sub_features,
                    bundle["splits"],
                    sub_params,
                    f"{label}_sector_only",
                    exp_root,
                    max_splits=MAX_SPLITS,
                    use_early_stopping=USE_EARLY_STOPPING,
                )
            ...
            payload = {
                ...
                "universal_on_fs": candidate_summary("universal_on_fs", universal_state, best_fs_params, {"model_type": "full_universe_fit_fs_subset_eval_aligned"}),
                ...
                "pass_vs_universal": bool(best_fs_state["aggregate"]["mean_test_ic"] > universal_state["aggregate"]["mean_test_ic"]),
            }
            write_json(exp_root / "exp13_v7_summary.json", payload)
            write_table(exp_root / "exp13_v7_subuniverses.csv", pd.DataFrame(subuniverse_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp14_v7():
            ...
            if anchor_feature is None:
                raise RuntimeError("An FX anchor feature must be present for EXP-14 v7.")
            ...
            write_json(exp_root / "exp14_v7_summary.json", payload)
            write_table(exp_root / "exp14_v7_importance_checks.csv", pd.DataFrame(importance_rows))
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp15_v7():
            ...
            write_json(exp_root / "exp15_v7_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp16_v7():
            ...
            write_json(exp_root / "exp16_v7_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
        ...
        def run_exp17_v7():
            ...
            write_json(exp_root / "exp17_v7_summary.json", payload)
        ...
        def run_exp18_v7():
            exp_slug = "exp18_v7"
            exp_root = experiment_dir(exp_slug)
            exp17_state = RESULT_CACHE.get("exp17_v7")
            if exp17_state is None:
                exp17_state = run_exp17_v7()
            event_windows = regime_event_windows()
            ...
            write_json(exp_root / "exp18_v7_summary.json", payload)
            RESULT_CACHE[exp_slug] = payload
            return payload
...
def main() -> None:
    ratio_repo = NOTEBOOK_DIR / "northstar_exp09_12_ratio_campaign_rerun_v7_20260407.ipynb"
    ratio_downloads = DOWNLOADS_DIR / "northstar_exp09_12_ratio_campaign_rerun_v7_20260407.ipynb"
    sector_repo = NOTEBOOK_DIR / "northstar_exp13_18_sector_regime_rerun_v7_20260407.ipynb"
    sector_downloads = DOWNLOADS_DIR / "northstar_exp13_18_sector_regime_rerun_v7_20260407.ipynb"
