...
Dataset: pranali96/Indian_Financial_News
"""
...
def download_dataset():
    """Download and save the pranali96 Indian Financial News dataset"""
    ...
    print("Dataset: pranali96/Indian_Financial_News")
    
    try:
        # Load the dataset
        ds = load_dataset("pranali96/Indian_Financial_News")
        ...
        for split_name, split_data in ds.items():
            ...
            # Save as CSV
            csv_path = data_dir / f"pranali_indian_financial_news_{split_name}.csv"
            split_data.to_csv(str(csv_path))
            print(f"Saved CSV: {csv_path}")
            
            # Save as JSON
            json_path = data_dir / f"pranali_indian_financial_news_{split_name}.json"
            split_data.to_json(str(json_path))
            print(f"Saved JSON: {json_path}")
            
            # Save as Parquet (efficient format)
            parquet_path = data_dir / f"pranali_indian_financial_news_{split_name}.parquet"
